#!/bin/bash
#
# Name:        sun-dump-sybase-coh-nfs.bash
#
...
# 06/16/26 Diana Yang   Use gfind, gawk
#
#################################################################
...
#trim log directory
gfind $DIR/log/$servicename/${dbname}* -type f -mtime +7 -exec /bin/rm {} \;
gfind $DIR/log/$servicename -type f -mtime +14 -exec /bin/rm {} \;
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$servicename/$dbname" ]]; then
    gfind ${mount}1/$servicename/$dbname -type f -mtime +$retnewday -exec /bin/rm -f {} \;
    gfind ${mount}1/$servicename/$dbname -depth -type d -empty -exec rmdir {} \;
  fi
