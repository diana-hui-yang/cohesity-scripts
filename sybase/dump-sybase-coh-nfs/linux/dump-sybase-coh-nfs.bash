...
#trim log directory
find $DIR/log/$servicename/${dbname}* -type f -mtime +7 -exec /bin/rm {} \;
find $DIR/log/$servicename -type f -mtime +14 -exec /bin/rm {} \;
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$servicename/$dbname" ]]; then
    find ${mount}1/$servicename/$dbname -type f -mtime +$retnewday -exec /bin/rm -f {} \;
