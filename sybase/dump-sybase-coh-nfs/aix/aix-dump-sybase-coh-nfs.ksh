#!/bin/ksh
#
# Name:         aix-dump-sybase-coh-nfs.ksh
#
...
# 06/16/26 Diana Yang	Convert bash to ksh
#
#################################################################
 
function show_usage {
echo "usage: dump-sybase-coh-nfs.ksh -U <sybase sa user> -P <password> -k <key> -S <service name> -d <database name> -t <db or log> -m <mount-prefix> -n <number of mounts> -p <number of stripes> -e <retention> -X <yes/no> -w <yes/no>"
echo " "
echo " Required Parameters"
echo " -k : sybase login key"
echo " -d : database name"
echo " -t : dump type. db means database dump, log means transactional dump"
echo " -m : mount-prefix (like /coh/sybase1)"
...
fullcommand="$@"
lencommand=0
for arg in $fullcommand; do
  ((lencommand=lencommand+1))
done
 
# Check required parameters
if [ -n "$mount" ] && [ -n "$dbname" ] && [ -n "$num" ] && [ -n "$retday" ] && [ -n "$dumptype" ]
then
  :
else
  show_usage
...
then
   ...
   if [ -n "$user" ] && [ -n "$password" ] && [ -n "$servicename" ]
   then
      :
   else
...
#trim log directory
find "$DIR/log/$servicename/${dbname}"* -type f -mtime +7 -exec /usr/bin/rm {} \; 2>/dev/null
find "$DIR/log/$servicename" -type f -mtime +14 -exec /usr/bin/rm {} \; 2>/dev/null
...
case "$retday" in
  ...
  *)
    ...
    if [ -d "${mount}1/$servicename/$dbname" ]; then
      find "${mount}1/$servicename/$dbname" -type f -mtime +$retnewday -exec /usr/bin/rm -f {} \;
      find "${mount}1/$servicename/$dbname" -depth -type d -empty -exec rmdir {} \;
    fi
