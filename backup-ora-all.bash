...
while [ $i -le $num ]; do
  if mountpoint -q "${mount}${i}"; then
    ...
    if [[ $i -le $parallel ]]; then
	   echo "allocate channel fs$i device type disk format = '$mount$i/$host/$oraclesid/%d-%T-s%s-p%p-%U';" >> $rmanfiled
	   echo "allocate channel fs$i device type disk format = '$mount$i/$host/$oraclesid/%d-%T-s%s-p%p-%U';" >> $rmanfilea
	fi
    i=$[$i+1]
  else
...
$rmanlogin log $rmanlog @$rmanfilea
