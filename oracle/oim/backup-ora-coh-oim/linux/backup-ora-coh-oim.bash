...
if [[ $ttype = "full" || $ttype = "Full" || $ttype = "FULL" ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
	...
	if [[ -n `ls -A ${mount}1/$host/$dbname/datafile` ]]; then
           find ${mount}1/$host/$dbname/datafile/* -type f -exec /bin/rm -v {} \; >> $runlog
	fi
     fi
...
#    find ${mount}1/$host/$dbname/* -type d -mtime +$retnewday -prune -exec rm -rv {} \; >> $runlog
    ...
    do
	  ...
	  find $i/* -type f -exec /bin/rm -v {} \; >> $runlog
	  /bin/rm -rv $i >> $runlog
       fi
    done
  ...
  if [[ -n $orighost && $orighost != $host ]]; then
     if [[ -d "${mount}1/$orighost/$origdbname" ]]; then
        ...
        if [[ -d ${mount}1/$orighost/$origdbname/datafile ]]; then
   	   ...
   	   find ${mount}1/$orighost/$origdbname/datafile/* -type f -mtime +30 -exec /bin/rm -v {} \; >> $runlog
        fi
