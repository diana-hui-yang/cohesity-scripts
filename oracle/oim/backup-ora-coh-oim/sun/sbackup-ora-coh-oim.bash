...
if [[ -z $targetc ]]; then
  ...
  if [[ $targetc == "/" ]]; then
    ...
    systype=`echo $targetc | gawk -F @ '{print $2}' | gawk 'NF>1{print $NF}'`
    if [[ -z $hostdefinded ]]; then
       if [[ $conn =~ '/' ]]; then
          host=`echo $conn | gawk -F '/' '{print $1}'`
          if [[ $host =~ ':' ]]; then
	           host=`echo $host | gawk -F ':' '{print $1}'`
	        fi
       fi
    fi  
    if [[ -z $systype ]]; then
    ...
    if [[ -z $dbname ]]; then
       if [[ $conn =~ '/' ]]; then
          dbname=`echo $conn | gawk -F '/' 'NF>1{print $NF}'`
       else
          dbname=$conn
       fi
    fi
...
EOF
   ...
   if [[ -n $dbcopyfile ]]; then
      dbsrcdir=`echo ${dbcopyfile} | gawk 'BEGIN{FS=OFS="/"}{NF--; print}'`
		if [[ -n $dbsrcdir ]]; then
         echo "The full backup directory is $dbsrcdir"
         echo "The full backup directory is $dbsrcdir" >> $runlog

         dbsrctopdir=`echo ${dbsrcdir} | gawk 'BEGIN{FS=OFS="/"}{NF--; print}'`
         origdbname=`echo ${dbsrctopdir} | gawk -F "/" '{print $NF}'` 
         orighost=`echo ${dbsrctopdir} | gawk 'BEGIN{FS=OFS="/"}{NF--; print}' | gawk -F "/" '{print $NF}'`
...
if [[ $ttype = "full" || $ttype = "Full" || $ttype = "FULL" ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
	...
	if [[ -n `ls -A ${mount}1/$host/$dbname/datafile` ]]; then
           gfind ${mount}1/$host/$dbname/datafile/* -type f -exec /bin/rm -v {} \; >> $runlog
	fi
     fi
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
 ...
 # let retnewday=$retday+1
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    ...
    echo direcory "${mount}1/$host/$dbname" exist >> $runlog
    if [[ -d ${mount}1/$host/$dbname/datafile ]]; then
       gfind ${mount}1/$host/$dbname/datafile/* -type f | grep -v D-${db_name^^}_ >> ${filelist}
...
#    gfind ${mount}1/$host/$dbname/* -type d -mtime +$retnewday -prune -exec rm -rv {} \; >> $runlog
    ...
    do
       createdtime=`echo $i | gawk -F "." '{print $2}'`
       if [[ $createdtime -le $expiretime ]]; then
	        echo "Delete directory $i" >> $runlog
	        gfind $i/* -type f -exec /bin/rm -v {} \; >> $runlog
	        /bin/rm -rv $i >> $runlog
       fi
    done
  ...
  if [[ -n $orighost && $orighost != $host ]]; then
     if [[ -d "${mount}1/$orighost/$origdbname" ]]; then
        ...
        if [[ -d ${mount}1/$orighost/$origdbname/datafile ]]; then
           gfind ${mount}1/$orighost/$origdbname/datafile/* -type f | grep -v D-${db_name^^}_ >> ${origfilelist}
        fi
        gfind ${mount}1/$orighost/$origdbname/archivelog/* -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
        gfind ${mount}1/$orighost/$origdbname/controlfile/* -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
        gfind ${mount}1/$orighost/$origdbname/spfile/* -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
#        gfind ${mount}1/$orighost/$origdbname/* -type d -mtime +$retnewday -prune -exec /bin/rm -rv {} \; >> $runlog
        echo "
        find the datafile.* directories that are created +$retnewday ago and delete them"
 ...
 #   filenum=`wc -l ${filelist} | gawk '{print $1}'`
        ...
        gfind ${mount}1/$orighost/$origdbname -depth -type d -empty -exec /bin/rmdir -v {} \; >> $runlog
