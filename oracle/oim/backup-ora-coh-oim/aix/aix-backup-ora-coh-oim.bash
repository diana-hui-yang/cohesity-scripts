#!/bin/bash
#
# Name:         aix-backup-ora-coh-oim.bash
#
...
# 01/07/22 Diana Yang   Modify Linux script to work in AIX
# 01/07/22 Diana Yang   change "awk" to "/opt/freeware/bin/gawk" 
# 01/07/22 Diana Yang   change "find" to "/opt/freeware/bin/find"
# 01/10/22 Diana Yang   change "/bin/rm" to "/opt/freeware/bin/rm"
...
# 10/21/24 Diana Yang   It will automatically detect if the database has been duplicated from another database or its backup. If so, it will initiate a full backup.
#
#################################################################

function show_usage {
echo "usage: aix-backup-ora-coh-oim.bash -r <Target connection> -c <Catalog connection> -h <host> -d <rac-node1-conn,rac-node2-conn,...> -o <Oracle_DB_Name> -t <backup type> -a <archive only> -y <Cohesity-cluster> -v <view> -u <Cohesity Oracle User> -g <AD Domain> -z <section size> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -s <python script directory> -l <archive log keep days> -b <ORACLE_HOME> -f <number of archive logs> -x <Max throughput> -i <yes/no> -w <yes/no>" 
echo " "
...
dbidfile=$DIR/log/$host/${dbname}_dbid.$DATE_SUFFIX
dbidfile_current=$DIR/log/$host/${dbname}_dbid
...
EOF
   ...
   if [[ -n $dbcopyfile ]]; then
      dbsrcdir=`echo ${dbcopyfile} | /opt/freeware/bin/gawk 'BEGIN{FS=OFS="/"}{NF--; print}'`
	  if [[ -n $dbsrcdir ]]; then
         echo "The full backup directory is $dbsrcdir"
         echo "The full backup directory is $dbsrcdir" >> $runlog

         dbsrctopdir=`echo ${dbsrcdir} | /opt/freeware/bin/gawk 'BEGIN{FS=OFS="/"}{NF--; print}'`
         origdbname=`echo ${dbsrctopdir} | /opt/freeware/bin/gawk -F "/" '{print $NF}'` 
         orighost=`echo ${dbsrctopdir} | /opt/freeware/bin/gawk 'BEGIN{FS=OFS="/"}{NF--; print}' | /opt/freeware/bin/gawk -F "/" '{print $NF}'`
...
if [[ -f $dbidfile_current ]]; then
   old_dbid=`cat $dbidfile_current`
   if [[ $new_dbid != $old_dbid ]]; then
      echo "The database DBID has been changed, and a full backup will be performed."
      echo "The database DBID has been changed, and a full backup will be performed." >> $runlog
      ttype=full
      cp $dbidfile $dbidfile_current
      echo "new DBID is $new_dbid"
      echo "new DBID is $new_dbid" >> $runlog
   fi
else
   cp $dbidfile $dbidfile_current
fi
...
if [[ $j -gt $num_dfile && $num_dfile -ne 200 ]]; then
   echo " tag '${tag}';" >> $catalog_bash
fi
...
function clean_datafile {
   
if [[ $ttype = "full" || $ttype = "Full" || $ttype = "FULL" ]]; then
  echo "Clean old backup files in datafile directory started at " `/bin/date '+%Y%m%d%H%M%S'`
  echo "Clean old backup files in datafile directory started at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
  if [[ -d "${mount}1/$host/$dbname" ]]; then
	...
	if [[ -n `ls -A ${mount}1/$host/$dbname/datafile` ]]; then
           /opt/freeware/bin/find ${mount}1/$host/$dbname/datafile/* -type f -exec /opt/freeware/bin/rm-v {} \; >> $runlog
	fi
     fi
	  
     if [ $? -ne 0 ]; then
        echo "Clean backup files in ${mount}1/$host/$dbname/datafile directory failed at " `/bin/date '+%Y%m%d%H%M%S'`
        echo "Clean backup files in ${mount}1/$host/$dbname/datafile directory failed at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
     else
        echo "Clean backup files in ${mount}1/$host/$dbname/datafile directory finished at " `/bin/date '+%Y%m%d%H%M%S'`
        echo "Clean backup files in ${mount}1/$host/$dbname/datafile directory finished at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
     fi
  else
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
 ...
 # let retnewday=$retday+1
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
	...
	echo direcory "${mount}1/$host/$dbname" exist >> $runlog
    if [[ -d ${mount}1/$host/$dbname/datafile ]]; then
       /opt/freeware/bin/find ${mount}1/$host/$dbname/datafile/* -type f | grep -v D-${db_name^^}_ >> ${filelist}
    ...
    /opt/freeware/bin/find ${mount}1/$host/$dbname/spfile/* -type f -mtime +$retnewday  -exec /opt/freeware/bin/rm -v {} \; >> $runlog
#    /opt/freeware/bin/find ${mount}1/$host/$dbname/* -type d -mtime +$retnewday -prune -exec /opt/freeware/bin/rm -rv {} \; >> $runlog
    ...
    do
       createdtime=`echo $i | /opt/freeware/bin/gawk -F "." '{print $2}'`
       if [[ $createdtime -le $expiretime ]]; then
	  echo "Delete directory $i" >> $runlog
	  /opt/freeware/bin/find $i/* -type f -exec /opt/freeware/bin/rm -v {} \; >> $runlog
	  /opt/freeware/bin/rm -rv $i >> $runlog
       fi
    done
 ...
 #   filenum=`wc -l ${filelist} | /opt/freeware/bin/gawk '{print $1}'`
	
    if [[ -f ${filelist} ]]; then
       while IFS= read -r line
       do
         /opt/freeware/bin/rm -v $line >> $runlog
    ...
    /opt/freeware/bin/find ${mount}1/$host/$dbname -depth -type d -empty -exec /opt/freeware/bin/rmdir -v {} \; >> $runlog
  ...
  if [[ -n $orighost && $orighost != $host ]]; then
     if [[ -d "${mount}1/$orighost/$origdbname" ]]; then
        ...
        if [[ -d ${mount}1/$orighost/$origdbname/datafile ]]; then
           /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/datafile/* -type f | grep -v D-${db_name^^}_ >> ${origfilelist}
        fi
        /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/archivelog/* -type f -mtime +$retnewday  -exec /opt/freeware/bin/rm -v {} \; >> $runlog
        /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/controlfile/* -type f -mtime +$retnewday  -exec /opt/freeware/bin/rm -v {} \; >> $runlog
        /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/spfile/* -type f -mtime +$retnewday  -exec /opt/freeware/bin/rm -v {} \; >> $runlog
#        /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/* -type d -mtime +$retnewday -prune -exec /opt/freeware/bin/rm -rv {} \; >> $runlog
        echo "
        find the datafile.* directories that are created +$retnewday ago and delete them"
...
#      /opt/freeware/bin/find ${mount}1/$orighost/$origdbname -depth -type d -empty -exec /opt/freeware/bin/rmdir -v {} \; >> $runlog
	   ...
	   topdir=${mount}1/$orighost/$origdbname
    else
...
datafile_newdir=$topdir/datafile.$DATE_SUFFIX
...
if [[ -z $apikey ]]; then 
  ...
  if [[ $? -eq 0 ]]; then
    ...
    if [[ $n -eq 6 ]]; then
       ...
       if [[ -d ${datafile_newdir} ]]; then 
         echo "Even the clone job returns none zero error code, the target directory ${datafile_newdir} was created"
       else
         exit 1
       fi 
    elif [[ $n -lt 6 ]]; then
      ...
      if [[ -d ${datafile_newdir} ]]; then 
        ...
        echo "will retry Cohesity snapshot again. $n"
        echo "will retry Cohesity snapshot again. $n" >> $runlog
        n=$[$n+1]
      fi
    fi
  fi
