...
#If it is a CDB database, need to find whether this datafile belongs to any PDB daatabase

if [[ $cdb == "yes" ]]; then

   echo "Find whether this datafile belongs to PDB\$SEED or belongs to a pluggable database that isn't open."
   echo "Find whether this datafile belongs to PDB\$SEED or belongs to a pluggable database that isn't open." >> $runlog
...
Restore $datafile_name will not start." >> $runlog
   ...
   echo "
   The datafile_name $datafile_name is in CDB or a PDB database
   Find whether this datafile belongs to a PDB database. If it does, find the PDB database name"
   echo "
   The datafile_name $datafile_name is in CDB or a PDB database
   Find whether this datafile belongs to a PDB database. If it does, find the PDB database name" >> $runlog
