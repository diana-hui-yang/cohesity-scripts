...
function show_usage {
echo "usage: export-ora-coh-nfs.bash -s <sqlplus connection> -h <host> -o <Oracle_sid> -r <remote sqlplus connection> -m <mount-prefix> -n <number of mounts> -d <number of export directories> -p <number of parallels> -e <retention> -z <file size> -x <ORACLE_HOME> -c <export options> -w <yes/no>"
echo " "
echo " Required Parameters"
echo " -h : host (scanname is required if it is RAC. optional if it is standalone.)"
echo " -o : ORACLE_DB_NAME (Need to have an entry of this database in /etc/oratab. If it is RAC, it is db_unique_name. It is CDB database.)"
...
if [[ -n $rsqlplusc ]]; then
   rcred=`echo $rsqlplusc | awk -F @ '{print $1}'`
   rconn=`echo $rsqlplusc | awk -F @ '{print $2}' | awk '{print $1}'`
   roralogin=`echo $rcred | awk -F / '{print $1}'`
   rpassword=`echo $rcred | awk -F / '{print $2}'`
