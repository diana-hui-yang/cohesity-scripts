#!/bin/bash
#
# Name:         restore-datafile-agent.bash
#
# Function:     This script restore Oracle datafiles from Oracle backup using Cohesity Oracle agent.
...
function show_usage {
echo "usage: restore-datafile-agent.bash -o <Oracle instance name> -f <datafile name or id> -m <mount>  -b <ORACLE_HOME> -w <yes/no>"
echo " "
...
if [[ -z $mount ]]; then
   read -p "The restore of the datafile will use the current Cohesity Adapter NFS mount and the lastest archivelog are still on disk on this server.
If the current Cohesity Adapter NFS is not mounted or the lastest archivelog aren't on disk, the restore of the datafile will fail.
