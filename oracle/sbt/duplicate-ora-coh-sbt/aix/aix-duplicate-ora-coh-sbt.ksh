#!/bin/ksh
#
# Name:         aix-duplicate-ora-coh-sbt.ksh
#
...
function show_usage {
echo "usage: aix-duplicate-ora-coh-sbt.ksh -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -n <new PDB name> -l <yes/no> -a <Auxiliary database> -f <yes/no> -m <noresume> -w <yes/no> -g <yes/no> -k <cert path>" 
echo " "
...
while getopts ":r:c:h:d:i:y:b:t:e:j:v:s:p:o:u:n:l:a:f:m:g:k:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
...
#echo itime is $itime sequence is $sequence
...
if [[ -n $sequenceset ]]; then
   if [[ -z $sequence ]]; then
      ...
      if echo "$sequence" | grep -qE '^[0-9]+$'; then
...
if [[ -n $targetc ]]; then
   rmanr=`$ORACLE_HOME/bin/rman << EOF
...
if [[ -n $catalogc ]]; then
   ...
   catar=`$ORACLE_HOME/bin/rman << EOF
...
#getting sqlplus login
   ...
   ssqllogin="$ORACLE_HOME/bin/sqlplus ${cred}@${conn} as $systype"
fi
...
stdout=$DIR/rmanlogs/$thost/$toraclesid.dup.$DATE_SUFFIX.std
pdbplug=$DIR/rmanlogs/$thost/$toraclesid.$pdbname.$DATE_SUFFIX.xml
drmanlog=$DIR/rmanlogs/$thost/$toraclesid.rman-duplicate.$DATE_SUFFIX.log
drmanfiled=$DIR/rmanlogs/$thost/$toraclesid.rman-duplicate.$DATE_SUFFIX.rcv
destroylog=$DIR/rmanlogs/$thost/$adbname.destroy.$DATE_SUFFIX.log
plug_bash=$DIR/rmanlogs/$thost/$adbname.plug.$DATE_SUFFIX.bash
pluglog=$DIR/rmanlogs/$thost/$toraclesid.plug.$DATE_SUFFIX.log
plug_aux_log=$DIR/rmanlogs/$thost/$toraclesid.aux.plug.$DATE_SUFFIX.log
unplug_bash=$DIR/rmanlogs/$thost/$adbname.unplug.$DATE_SUFFIX.bash
unpluglog=$DIR/rmanlogs/$thost/$adbname.unplug.$DATE_SUFFIX.log
destroy_aux_bash=$DIR/rmanlogs/$thost/$adbname.delete_aux.$DATE_SUFFIX.bash
destroy_aux_log=$DIR/rmanlogs/$thost/$adbname.delete_aux.$DATE_SUFFIX.log

#trim log directory
#find $DIR/rmanlogs/$thost -type f -mtime +7 -exec /bin/rm {} \;

#if [ $? -ne 0 ]; then
#  echo "del old logs in $DIR/rmanlogs/$thost failed"
#  exit 2
#fi
...
EOF
   ...
   do
     if [[ $i -ge 1 ]]; then
        ...
        case $snode in
           *.*)
             snodeshort=`echo $shost | awk -F '.' '{print $1}'`
             ;;
           *)
             snodeshort=$snode
...
backupdir=`$DIR/tools/sbt_list --view=$view/$shostshort --vips=$ip | grep -i ${sdbname}`
if [[ -z ${backupdir} ]]; then
   backupdir=`$DIR/tools/sbt_list --view=$view/$shost --vips=$ip | grep -i ${sdbname}`
   if [[ -z ${backupdir} ]]; then
      echo "Error: The directory ${shost}/${sdbname} or ${shostshort}/${sdbname} doesn't exist in view ${view} of this Cohesity Cluster. Please check the input"
      exit 2
   fi
else
   echo "The backup diretory is ${shostshort}/${sdbname} in the view ${view}"
   shost=$shostshort
fi
...
EOF
   ...
   if [[ $force != [Yy]* ]]; then
	 ...
	 echo "Running duplication may overwrite the existing data"
         echo " "
         echo "Please type yes if you want to continue this duplication? \c"
         read answer4
         if [[ $answer4 != [Yy]* ]]; then
            echo "
...
#     echo ora_pfile is ${ora_pfile}
      if [[ -z `grep -i db_create_file_dest $dbpfile` && -z `grep -i db_file_name_convert $dbpfile` ]]; then
         ...
         echo " "
         echo "Continue MAY overwrite the target database files. Do you want to continue? \c"
         read answer1
         if [[ $answer1 = [Nn]* ]]; then
            exit
         fi
...
if [[ -z $pdbname ]]; then
  if [[ -n $ora_pfile ]]; then
    echo "ora_pfile is $ora_pfile"
    db_location=`grep -i newname $ora_pfile | grep -v "#" | awk -F "'" '{print $2}' | awk -F "%" '{print $1}'`
    set -A arrdbloc $db_location
    lendbloc=${#arrdbloc[@]}
	 
	i=0
	while [ $i -lt $lendbloc ]; do
...
# get the directory
       newdbdir=`echo ${arrdbloc[$i]} | awk 'BEGIN {FS="/"; OFS="/"} {sub("/[^/]*$", ""); print}'`
       echo $newdbdir
# check whether it is ASM or dirctory
...
EOF
      ...
      if [[ -n $pdbname ]]; then
	 if [[ -z $adbname ]]; then
             echo "Auxiliary database is not provided. Should target database ${ORACLE_SID} be refreshed with new data? \c"
             read answer2
	 else
             echo "Auxiliary database is up, should database ${ORACLE_SID} be refreshed with new data? \c"
             read answer2
	 fi
      else
         echo "Oracle database is up, Should target database ${ORACLE_SID} be refreshed with new data? \c"
         read answer2
      fi
      if [[ $answer2 = [Nn]* ]]; then
...
while [ $j -lt $parallel ]; do
   while IFS= read -r ip; do
      ...
      if [[ -n $ip ]]; then
	    ...
	    if [[ $encryption = [Yy]* ]]; then
               allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';"
	    else
               allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';"
            fi			   
         fi
         j=$((j + 1))
      fi
...
cp ${ORACLE_HOME}/dbs/init${adbname}.ora $DIR/rmanlogs/$thost/init${adbname}.save.ora
...
if grep -iq "error" "$drmanlog"; then
  if grep -iq "c0 not allocated" $drmanlog; then
...
if [[ $ora_license = "STANDARD" ]]; then
   echo "
Since the Oracle edition is Standard, only one RMAN channel will be used to run the duplicate command 
"
   parallel=1
fi
if [[ $preview != [Yy]* ]]; then
   ...
   if [[ $force = [Yy]* ]]; then
      ...
      echo "The init file $dbpfile needs to be created first. The successful of this duplication depends on the content of that file.
	       "
      if [[ -z $pdbname ]]; then
         echo "It is reommended to clean up the the files associated with database $toraclesid first. Please type yes if you want to continue this duplication? \c" 
         read answer3 
         if [[ $answer3 != [Yy]* ]]; then
            exit
         fi
...
if [[ $preview = [Yy]* ]]; then
   ...
   if [[ $force = [Yy]* ]]; then
      ...
      if [[ -n $adbname ]]; then
         ...
         $unplug_bash		 
         if [[ -z $pdbcopy ]]; then
             echo "This plug operation will not move the database file structure from auxiliary database to target database. Please type yes if you want to continue this plug operation? \c"
             read answer5
             if [[ $answer5 != [Yy]* ]]; then
