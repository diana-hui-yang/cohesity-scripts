...
#getting sqlplus login
   ...
   if [[ -z $shost ]]; then
      if [[ $conn =~ '/' ]]; then
         shost=`echo $conn | /opt/freeware/bin/gawk -F '/' '{print $1}'`
         if [[ $shost =~ ':' ]]; then
            shost=`echo $shost | /opt/freeware/bin/gawk -F ':' '{print $1}'`
         fi
      else
   ...
   if [[ $shost =~ '.' ]]; then
       shostshort=`echo $shost | /opt/freeware/bin/gawk -F '.' '{print $1}'`
   else
       shostshort=$shost
   fi
   if [[ -z $systype ]]; then
...
EOF
   ...
   do
     if [[ $i -ge 1 ]]; then
        snode=`echo $line | xargs | /opt/freeware/bin/gawk -F ":" '{print $1}'`
	if [[ $snode =~ '.' ]]; then
           snodeshort=`echo $snode | /opt/freeware/bin/gawk -F '.' '{print $1}'`
        else
	   snodeshort=$snode
...
EOF
      ...
      do
         if [[ $i -eq 4 ]]; then
            sequence=`echo $line | /opt/freeware/bin/gawk '{print $1}' | xargs`
	    thread=`echo $line | /opt/freeware/bin/gawk '{print $2}' | xargs`
            break 
         fi
         i=$[$i+1]
      done < $stdout 
...
runoid=`ps -ef | grep pmon | /opt/freeware/bin/gawk 'NF>1{print $NF}' | grep -i $ORACLE_SID | /opt/freeware/bin/gawk -F "pmon_" '{print $2}'`
...
while [ $j -lt $parallel ]; do
   while IFS= read -r ip; do
      ...
      if [[ -n $ip ]]; then
	    ...
	    if [[ $encryption = [Yy]* ]]; then
	       if [[ $grpctype = [Yy]* ]]; then
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrycert})';"
               else
		  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';"
	       fi
	    else
	       if [[ $grpctype = [Yy]* ]]; then
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)';"
               else
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false)';"
               fi
            fi			   
         fi
         j=$[$j+1]
