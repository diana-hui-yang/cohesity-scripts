...
function show_usage {
echo "usage: restore-ora-coh-sbt-23.bash -h <backup host> -c <Catalog connection> -i <Oracle instance name> -d <Oracle_DB_Name> -y <Cohesity-cluster> -t <point-in-time> -l <yes/no> -j <vip file> -v <view> -q <catalog view> -s <ZDLRA package directory> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>"
echo " "
...
while getopts ":h:c:i:d:t:y:l:j:v:q:s:p:o:f:g:k:x:w:" opt; do
  case $opt in
    h ) shost=$OPTARG;;
    c ) catalogc=$OPTARG;;
    i ) oraclesid=$OPTARG;;
...
if [[ ! -d $DIR/rmanlogs/$thost ]]; then
  echo " $DIR/rmanlogs/$thost does not exist, create it"
  mkdir -p $DIR/rmanlogs/$thost
  
  if [ $? -ne 0 ]; then
    echo "Error: create log directory $DIR/rmanlogs/$thost failed. There is a permission issue"
    exit 1
  fi
fi
