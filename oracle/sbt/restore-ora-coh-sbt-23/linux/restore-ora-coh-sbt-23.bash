...
if test -f $sbtname; then
   ...
   if test ! -f $sbtname; then
      echo "file $sbtname also does not exist. exit"
      exit 1
   fi
fi

if [[ ! -f ${zdlradir}/cohesity_sbt/bin/sbt_list ]]; then
   echo "${zdlradir}/cohesity_sbt/bin/sbt_list file does not exist."
   exit 1
fi
...
#echo $ip
${zdlradir}/cohesity_sbt/bin/sbt_list --view=$cata_view --vips=$ip | sort -nk1 | grep -i ${dbname}_c- > ${controlfile_list}
