...
# 07/01/24 Diana Yang	Change it to work with Cohesity ZDLRA package
# 07/15/24 Diana Yang	Restore the RAC database using all RAC nodes
# 01/28/26 Diana Yang	Add support to Oracle database with Standard Edition license (SE)
#
#################################################################

function show_usage {
echo "usage: restore-ora-coh-sbt.bash -h <backup host> -r <RAC database connection> -n <rac-node1-conn,rac-node2-conn,...> -c <Catalog connection> -i <Oracle instance name> -d <Oracle_DB_Name> -y <Cohesity-cluster> -t <point-in-time> -l <yes/no> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>"
echo " "
...
echo " -r : RAC database connection. Database restore will be multi-nodes if this parameter is provided  (example: \"<dbuser>/<dbpass>@<target connection string> as sysbackup\" )"
echo " -n : Rac nodes connectons strings that will be used to do restore (example: \"<rac1-node connection string,rac2-node connection string>\")"
...
echo " -w : yes means preview rman restore scripts"
}

while getopts ":h:r:n:c:i:d:t:y:l:j:v:s:p:o:f:g:k:x:w:" opt; do
  case $opt in
    r ) dbconn=$OPTARG;;
    h ) shost=$OPTARG;;
    c ) catalogc=$OPTARG;;
...
if [[ -n $racconns ]]; then
  ...
  echo  Node RAC connection is $racconns
fi 

if [[ -n $dbconn ]]; then
   cred=`echo $dbconn | awk -F @ '{print $1}'`
   conn=`echo $dbconn | awk -F @ '{print $2}' | awk '{print $1}'`
   systype=`echo $dbconn | awk -F @ '{print $2}' | awk 'NF>1{print $NF}'`
   if [[ -z $systype ]]; then
      systype=sysdba
   fi
fi
...
if [[ $encryption = [Yy]* ]];then
   echo "This restore will use encryption-in-flight"
   if [[ -z $encrydir ]]; then
      encrydir=$DIR/lib
   fi
...
if [[ -z $itime ]]; then
  ...
  con_ptime=`/bin/date -d "$itime + 2 days" '+%Y%m%d%H%M%S'`
...
$sqllogin << EOF > /dev/null
   ...
   SELECT
  CASE
    WHEN banner LIKE '%Enterprise Edition%' THEN 'ENTERPRISE'
    WHEN banner LIKE '%Standard Edition%'   THEN 'STANDARD'
    ELSE 'UNKNOWN'
  END AS edition
FROM v\$version
...
ora_license=`echo $ora_license | xargs`
echo Oracle Edition is $ora_license

}

function check_rac_node_input {

echo "
Check the RAC node connection string which is the input of argument -n. 
"
...
if [[ $open_mode != "MOUNTED" && $open_mode != "READ WRITE" ]]; then
   echo "Oracle database $oraclesid is not in mount or open mode"
   echo "start the database in mount mode"
   srvctl stop database -db $dbname
   srvctl start database -db $dbname -o mount
fi

echo "If the job hangs, that means the connection string is not correct. Type Ctcl+C
"
...
EOF
       ...
       if [[ $db_name == ${dbname^^} ]]; then
          ...
          echo "connection string ${oldarrconns[$k]} is not valid or the instance is down, the restore will not use this connection string"
       fi
       if [[ -f $stdout ]]; then
          rm $stdout
       fi
   ...
   if [[ ${#arrconns[@]} -eq 0 ]]; then
      echo "there is no valid connection string. Restore won't run"
      exit
   fi
fi
...
if [[ ! -f $ORACLE_HOME/dbs/init${oraclesid}.ora ]]; then
   echo "The oracle pfile $ORACLE_HOME/dbs/init${oraclesid}.ora doesn't exist. Please make sure the instance name is correct.
   "
   read -p "If you know the instance is correct and want to continue without the pfile, Type YES or yes to continue or any other keys to cancel: " answer5
   if [[ $answer5 != [Yy]* ]]; then
	  exit 2
   fi
fi
...
# Find the latest control file backup if the restore time is the current time. Otherwise, look for the control file backup that is two days after the restore time, if it exists.
while IFS= read -r line; do
   line=`echo $line | xargs`
   bitime=`echo $line | awk '{print $1}'`
   btime=`/bin/date -d "${bitime/-/ }" '+%Y%m%d%H%M%S'`
   if [[ $con_ptime -lt $btime ]]; then
...
echo "will create spfile rman restore script"
...
if [[ $encryption = [Yy]* ]]; then
   if [[ $grpctype = [Yy]* ]]; then
      echo "
set dbid $dbid;
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view/${backupdir},vips=$ip,sbt_certificate_file=${encrycert})';
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view/${backupdir},vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${cert})';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view/${backupdir},vips=$ip)';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view/${backupdir},vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
echo "will create controlfile rman restore script"
if [[ $encryption = [Yy]* ]]; then
   if [[ $grpctype = [Yy]* ]]; then
      echo "
set dbid $dbid;
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view/${backupdir},vips=$ip,sbt_certificate_file=${cert})';
...
if [[ $encryption = [Yy]* ]]; then
   if [[ $grpctype = [Yy]* ]]; then
      echo "
set dbid $dbid;
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '$shost/$db_name/%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '$shost/$db_name/%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-value=false,sbt_certificate_file=${encrycert})';
restore spfile FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '$shost/$db_name/%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';
restore spfile FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '$shost/$db_name/%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';
restore spfile FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
set CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE sbt_tape TO '$shost/$db_name/%d_%F.ctl';
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';
restore CONTROLFILE FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $controlfile_rmanfile
...
if [[ $encryption = [Yy]* ]]; then
   if [[ $grpctype = [Yy]* ]]; then
       echo "ALLOCATE CHANNEL FOR MAINTENANCE DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';
CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';" >> $restore_rmanfile
       if [[ $parallel -eq 1 ]]; then
	      echo "CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM 1 BACKUP TYPE TO BACKUPSET;" >> $restore_rmanfile
	   fi
	   if [[ $parallel -gt 1 ]]; then
          echo "RUN {" >> $restore_rmanfile
	   fi
   else 
       echo "ALLOCATE CHANNEL FOR MAINTENANCE DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';
CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';" >> $restore_rmanfile
	   if [[ $parallel -eq 1 ]]; then
	      echo "CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM 1 BACKUP TYPE TO BACKUPSET;" >> $restore_rmanfile
	   fi
	   if [[ $parallel -gt 1 ]]; then
   ...
   if [[ $grpctype = [Yy]* ]]; then
       echo "ALLOCATE CHANNEL FOR MAINTENANCE DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';
CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';" >> $restore_rmanfile
	   if [[ $parallel -eq 1 ]]; then
	      echo "CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM 1 BACKUP TYPE TO BACKUPSET;" >> $restore_rmanfile
	   fi
	   if [[ $parallel -gt 1 ]]; then
       ...
       echo "ALLOCATE CHANNEL FOR MAINTENANCE DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';
CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';" >> $restore_rmanfile
	   if [[ $parallel -eq 1 ]]; then
	      echo "CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM 1 BACKUP TYPE TO BACKUPSET;" >> $restore_rmanfile
	   fi
	   if [[ $parallel -gt 1 ]]; then
	 ...
	 if [[ -n $racconns ]]; then
	       ...
	       if [[ $encryption = [Yy]* ]]; then
	          if [[ $grpctype = [Yy]* ]]; then
	             ...
	             allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';"
	          fi
               else
	          if [[ $grpctype = [Yy]* ]]; then
                     ...
                     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';"
                  fi
               fi
	       ...
	       if [[ $encryption = [Yy]* ]]; then
	          if [[ $grpctype = [Yy]* ]]; then
	             ...
	             allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';"
	          fi
               else
	          if [[ $grpctype = [Yy]* ]]; then
                     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';"
                  else
                     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';"
                  fi
               fi		  
               unallocate[$j]="release channel c$j;"
               j=$[$j+1]
...
Create rman reoover database file"
...
function open_cdb_pdb {

if [[ -n $dbconn ]]; then
   echo "This is a RAC databae, will stop and open the database on all RAC nodes
   "
   srvctl stop database -db $dbname
  ...
  if [ $? -ne 0 ]; then
  echo "   "
  echo "database $dbname failed to restarted "
  exit 1
else
  echo "  "
  echo "database $dbname restarted successfully"
fi
...
if [[ $pdbname =~ 'PDB' ]]; then
   echo "
   This database is a multi-tenant database. Will start all pluggable databases
   "
   if [[ -n $dbconn ]]; then
      $sqllogin << EOF
...
EOF
   ...
   echo "
   This database is a not a multi-tenant database.
   "
fi
...
if [[ -n $dbconn ]]; then
   echo "A oracle connect string is provided. This restore will use all nodes of the database"
   targetc=$dbconn
fi
...
if [[ -n $fra ]]; then 
   if [[ ${fra:0:1} != "+" ]]; then
      ...
      dirsize=`du -sk $fra | awk '{print $1}'`

      if [[ $dirsize != 0 ]]; then 
         read -p "The recovery area $fra is not empty. Please clean the directory, then Type YES or yes to continue: " answer4
...
setup
check_oracle_license

if [[ $ora_license = "STANDARD" ]]; then
   echo "
Since the Oracle edition is Standard, only one RMAN channel will be used for the restore
"
   parallel=1
fi
...
if [[ $preview = "yes" || $preview = "Yes" || $preview = "YES" ]]; then
   ...
   echo "   "
   echo ORACLE recovery RMAN SCRIPT 
   echo " "
   echo "---------------"
...
"
   ...
   read -p "Any data in the FRA area for this database should be deleted. Type YES or yes to continue or any other keys to cancel: " answer4
   if [[ $answer4 = [Yy]* ]]; then
      if [[ $host == $shost ]]; then
