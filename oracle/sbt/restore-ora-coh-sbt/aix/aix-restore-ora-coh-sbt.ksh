#!/bin/ksh
#
# Name:         aix-restore-ora-coh-sbt.ksh
#
...
function show_usage {
echo "usage: aix-restore-ora-coh-sbt.ksh -h <backup host> -c <Catalog connection> -i <Oracle instance name> -d <Oracle_DB_Name> -y <Cohesity-cluster> -t <point-in-time> -l <yes/no> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>"
echo " "
...
if [[ -n $grpctypeset ]]; then
   if [[ -z $grpctype ]]; then
      ...
      if [[ $grpctype != [Yy]* && $grpctype != [Nn]* ]]; then
         echo "SBT on AIX doesn't support SunRPC. Please enter 'yes' as the argument for -x"
	 exit 2
      fi
   fi
else
...
if [[ -n $encryptionset ]]; then
   if [[ -z $encryption ]]; then
	 ...
	 exit 2
      ...
      echo "SBT on AIX currently doesn't support encrytion in flight. Only 'no' is taken"
      encryption=no
   fi 
fi
...
rmanlogin="$ORACLE_HOME/bin/rman target /"
echo "rman login command is $rmanlogin"

targetc="/"
sqllogin="$ORACLE_HOME/bin/sqlplus / as sysdba"
...
if [[ -z $itime ]]; then
  ...
  ptime=`echo "$itime" | awk '{print $1$2}' | sed 's/[-:]//g'`
  con_ptime=$(perl -MPOSIX -e '
  use Time::Local;
  my $itime = shift;
  my ($year, $mon, $mday, $hour, $min, $sec) = unpack("A4A2A2A2A2A2", $itime);
...
while IFS= read -r line; do
   ...
   btime=`echo $bitime | awk -F '-' '{print $1$2}' | sed 's/[/:]//g'`
...
EOF
   ...
   echo "spfile restore started at " `date '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
  echo "   "
  echo "restore controlfile $controlfile failed at " `date '+%Y%m%d%H%M%S'`
  ...
  echo "  "
  echo "restore controlfile finished at " `date '+%Y%m%d%H%M%S'`
fi
...
echo "Database restore validate started at " `date '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
    echo "   "
    echo "Database restore validate failed at " `date '+%Y%m%d%H%M%S'`
    ls -l ${ORACLE_HOME}/dbs/spfile*
	echo "The last 10 line of rman log output"
    echo " "
    ...
    echo "  "
    echo "Database restore validate finished at " `date '+%Y%m%d%H%M%S'`
fi
...
echo "Database restore started at " `date '+%Y%m%d%H%M%S'`
echo "ORACLE SID is $oraclesid"
if [[ -f $stdout ]]; then
   rm $stdout
fi
...
EOF
   ...
   if [ $? -ne 0 ]; then
     echo "   "
     echo "Database restore failed at " `date '+%Y%m%d%H%M%S'`
     ls -l ${ORACLE_HOME}/dbs/spfile*
     echo "The last 10 line of rman log output"
     echo " "
     echo "rmanlog file is $restore_rmanlog"
     ...
     echo "  "
     echo "Database restore finished at " `date '+%Y%m%d%H%M%S'`
   fi
else
   echo "Oracle database $oraclesid is in mount open mode"
   if [[ $force = [Yy]* ]]; then
     if [[ $full != [Yy]* ]]; then
       echo "The restore will overwrite the database $dbname. Type YES or yes to confirm or any other keys to cancel: \c"
       read answer3
     else
       answer3=yes
     fi
...
"
   ...
   echo "Any data in the FRA area for this database should be deleted. Type YES or yes to continue or any other keys to cancel: \c"
   read answer4
   if [[ $answer4 = [Yy]* ]]; then
      if [[ $host == $shost ]]; then
         echo "The database $dbname will be overwritten. Type YES or yes to confirm or any other keys to cancel: \c"
         read answer2
         if [[ $answer2 = [Yy]* ]]; then
            if [[ -n $catalogc ]]; then
               create_rman_restore_controlfile_catalog
