...
function show_usage {
echo "usage: duplicate-ora-coh-sbt-23.bash -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -q <catalog view> -s <ZDLRA package directory>> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -n <new PDB name> -l <yes/no> -a <Auxiliary database> -f <yes/no> -m <noresume> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
while [ $j -lt $parallel ]; do
   while IFS= read -r ip; do
      ...
      if [[ -n $ip ]]; then
	    ...
	    if [[ $encryption = [Yy]* ]]; then
	       if [[ $grpctype = [Yy]* ]]; then
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,catalog_view=$cata_view,vips=$ip,sbt_certificate_file=${encrycert})';"
               else
		  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,catalog_view=$cata_view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';"
	       fi
	    else
	       if [[ $grpctype = [Yy]* ]]; then
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,catalog_view=$cata_view,vips=$ip)';"
               else
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,catalog_view=$cata_view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=false)';"
               fi
            fi			   
         fi
         j=$[$j+1]
