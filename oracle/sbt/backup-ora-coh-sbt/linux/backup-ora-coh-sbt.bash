...
# 01/06/26 Diana Yang   Add special retention to long term retention backups like monthly backups and yearly backups
#
#################################################################
...
echo " -R : Long Term Retention time (days to retain this particular backups, need RMAN catalog database. )"
...
while getopts ":r:c:h:n:o:a:i:y:f:v:s:p:e:R:l:b:z:k:m:t:g:j:x:d:u:q:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
if test $retday && test $sretday
then
  echo "Either provide the -e option or -u option; both cannot be used together."  
  exit 1
fi

if test $longretday && test $sretday
then
  echo "Either provide the -R option or the -u option; both cannot be used together. The -R option works only with the -e option."
  exit 1
fi
	      ...
	      if [[ $encryption = [Yy]* ]]; then
	         if [[ -n $cohesityname ]]; then
	            if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_certificate_file=${encrycert})';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		   fi
		    ...
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_use_grpc=false)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';" >> $rmanfiled
		    fi
		 fi
	      fi
...
#                   Delete noprompt obsolete device type sbt;" >> $rmanfiled
	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
                       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then 
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true);" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
	  	 fi
		 unallocate[j]="release channel c$j;"
	      ...
	      if [[ $j -lt $parallel ]]; then
                 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
		 fi
	         unallocate[j]="release channel c$j;"
              fi
...
if [[ -n $longretday ]]; then
  if [[ $level = "offline" || $level = "full" ]]; then
    if [[ -z $sectionsize ]]; then
      if [[ $compression = [Yy]* ]]; then
         echo "backup AS COMPRESSED BACKUPSET TAG '$TAG' database filesperset 1 format './$host/$db_name/%d_%T_%U_$level.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      else
         echo "backup TAG '$TAG' database filesperset 1 format './$host/$db_name/%d_%T_%U_$level.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      fi
    else
      if [[ $compression = [Yy]* ]]; then
         echo "backup AS COMPRESSED BACKUPSET TAG '$TAG' database section size ${sectionsize}G filesperset 1 format './$host/$db_name/%d_%T_%U_$level.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      else
         echo "backup TAG '$TAG' database section size ${sectionsize}G filesperset 1 format './$host/$db_name/%d_%T_%U_$level.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      fi
    fi
  else
    if [[ -z $sectionsize ]]; then
      if [[ $compression = [Yy]* ]]; then
         echo "backup AS COMPRESSED BACKUPSET INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database filesperset 1 format './$host/$db_name/%d_%T_%U_level${level}.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      else
         echo "backup INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database filesperset 1 format './$host/$db_name/%d_%T_%U_level${level}.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      fi
    else
      if [[ $compression = [Yy]* ]]; then
         echo "backup backup AS COMPRESSED BACKUPSET INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database section size ${sectionsize}G filesperset 1 format './$host/$db_name/%d_%T_%U_level${level}.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      else
         echo "backup INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database section size ${sectionsize}G filesperset 1 format './$host/$db_name/%d_%T_%U_level${level}.bdf' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
      fi
    fi
  fi
else
...
#fi
if [[ -n $longretday ]]; then
  if [[ $archlongretday = [Nn]* ]]; then
    if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    else
      echo "backup TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    fi
  elif perl -e "if ($archretday == 0) { exit 0 } else { exit 1 }"; then
    if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times delete input KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    else
      echo "backup TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times delete input KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    fi
  else
    if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    else
      echo "backup TAG '$archive_tag' archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfilea
    fi
  fi
else
...
if [[ -n $longretday ]]; then
   echo "BACKUP TAG '$ctl_tag' CURRENT CONTROLFILE format './$host/$db_name/%d_%T_%U.ctl' KEEP UNTIL TIME 'SYSDATE+$longretday';" >> $rmanfiled
else
...
while [ $j -lt $parallel ]; do
	...
	if [[ -n $ip ]]; then
	      ...
	      if [[ $encryption = [Yy]* ]]; then
	         if [[ -n $cohesityname ]]; then
	            if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		   fi
	         ...
	         if [[ -n $cohesityname ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    fi
		 fi
	      fi
#	         echo "crosscheck backup device type sbt;
...
#	           Delete noprompt obsolete device type sbt;" >> $rmanfiled
	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
                       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then 
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,log_level=0)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,log_level=0);" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
	  	 fi
		 unallocate[j]="release channel c$j;"
	         k=$[$k+1]
	      ...
	      if [[ $j -lt $parallel ]]; then
                 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,log_level=0)';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
		 fi
	         unallocate[j]="release channel c$j;"
              fi
...
if [[ $grpctype = [Yy]* ]]; then
   echo "sbt_list will use gRPC protocol"
   if [[ -n $cohesityname ]]; then
   ...
   echo "sbt_list will use SunRPC protocol"
   if [[ -n $cohesityname ]]; then
      $DIR/tools/sbt_list --view=$view/$host/$db_name --vips=$cohesityname --purge_view=true --days_before=$sretnewday --silent_mode=true -sbt_use_grpc=false >> $sbtlistlog
      olddata=`$DIR/tools/sbt_list --view=$view/$host/$dbname --vips=$cohesityname`
      if [[ -n $olddata ]]; then
         $DIR/tools/sbt_list --view=$view/$host/$dbname --vips=$cohesityname --purge_view=true --days_before=$sretnewday --silent_mode=true -sbt_use_grpc=false >> $sbtlistlog
      fi
   else
      $DIR/tools/sbt_list --view=$view/$host/$db_name --vips=$lastip --purge_view=true --days_before=$sretnewday --silent_mode=true -sbt_use_grpc=false >> $sbtlistlog
      olddata=`$DIR/tools/sbt_list --view=$view/$host/$dbname --vips=$lastip`
      if [[ -n $olddata ]]; then
         $DIR/tools/sbt_list --view=$view/$host/$dbname --vips=$lastip --purge_view=true --days_before=$sretnewday --silent_mode=true -sbt_use_grpc=false >> $sbtlistlog
      fi
   fi
fi
