	      ...
	      if [[ $encryption = [Yy]* ]]; then
	         if [[ -n $cohesityname ]]; then
	            if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_certificate_file=${encrycert})';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert})';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';" >> $rmanfiled
		   fi
		    ...
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_use_grpc=false)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';" >> $rmanfiled
		    fi
		 fi
	      fi
...
#                   Delete noprompt obsolete device type sbt;" >> $rmanfiled
	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
	  	 fi
		 unallocate[j]="release channel c$j;"
	      ...
	      if [[ $j -lt $parallel ]]; then
                 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert})';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert})' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
		 fi
	         unallocate[j]="release channel c$j;"
              fi
...
while [ $j -lt $parallel ]; do
	...
	if [[ -n $ip ]]; then
	      ...
	      if [[ $encryption = [Yy]* ]]; then
	         if [[ -n $cohesityname ]]; then
	            if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';" >> $rmanfiled
		   fi
	         ...
	         if [[ -n $cohesityname ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,log_level=0)';" >> $rmanfiled
	            elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" >> $rmanfiled
		    fi
		 fi
	      fi
#	         echo "crosscheck backup device type sbt;
...
#	           Delete noprompt obsolete device type sbt;" >> $rmanfiled
	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,log_level=0)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]} as $systype' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
	  	 fi
		 unallocate[j]="release channel c$j;"
	         k=$[$k+1]
	      ...
	      if [[ $j -lt $parallel ]]; then
                 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,sbt_certificate_file=${encrycert},log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,log_level=0)';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_use_grpc=false,log_level=0)' format './$host/$db_name/%d_%T_%U.blf';"
		    fi
		 fi
	         unallocate[j]="release channel c$j;"
              fi
