...
#	  echo "Delete obsolete;" >> $rmanfiled
	  ...
	  echo "RUN {" >> $rmanfilea
       ...
       if [[ -n $racconns ]]; then
          if [[ $j -lt $parallel ]]; then
             ...
             allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)' format '$host/$dbname/%d_%T_%U.blf';"
	  ...
	  if [[ $j -lt $parallel ]]; then
	     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)';"
   	     allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(data_view=$view,vips=$ip)' format '$host/$dbname/%d_%T_%U.blf';"
	     unallocate[j]="release channel c$j;"
          fi
          j=$[$j+1]
       fi
