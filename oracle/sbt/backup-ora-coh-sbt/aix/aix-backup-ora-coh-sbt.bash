	  ...
	  echo "RUN {" >> $rmanfilea
       ...
       if [[ -n $racconns ]]; then
          if [[ $j -lt $parallel ]]; then
             ...
             allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)' format './$host/$db_name/%d_%T_%U.blf';"
