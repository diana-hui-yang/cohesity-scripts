...
echo " -f : Yes means the script will execute restoreOracle-v2.py with the input; the default will just display the script."
echo " -d : Yes means the database to be refreshed will be dropped without prompting; the default will require confirmation."
echo " "
}
...
if [[ $execute != [Yy]* ]]; then
   ...
   if [ $? -ne 0 ]; then
     ...
     echo "Restoring database $oraclesid from the backup of database $origdbname job has failed to start." 
     echo "Restoring database $oraclesid from the backup of database $origdbname job has failed to start." >> $runlog
     exit 1
   else
     echo "python script finished at " `/bin/date '+%Y%m%d%H%M%S'`
     echo "python script finished at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
     echo "Restoring database $oraclesid from the backup of database $origdbname job started successfully."
     echo "Restoring database $oraclesid from the backup of database $origdbname job started successfully." >> $runlog
   fi
fi
