...
function show_usage {
echo "usage: create-standby-cohesityadapter.bash -i <Standby_instance_name> -u <Standby_unique_name> -d <db_name> -m <mount-prefix> -n <number of mounts> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -s <noresume> -c <nofilenamecheck> -w <yes/no>" 
echo " "
...
echo " -u : Standby Unique Name if it is different from the Standby Instance name" 
...
echo " -c : yes mean Oracle duplicate use nofilenamecheck, default is no"
echo " -w : yes means preview rman duplicate scripts"
echo "
"
}

while getopts ":i:u:d:m:n:p:o:f:s:c:w:" opt; do
  case $opt in
    d ) sdbname=$OPTARG;;
    i ) toraclesid=$OPTARG;;
    u ) uniquename=$OPTARG;;
...
for (( i=0; i<$lencommand; i++ ))
do
   if [[ ${fullcommand[$i]} == -c ]]; then
      nofilenamecheckset=yes
   fi
done
if [[ -n $nofilenamecheckset ]]; then
   if [[ -z $nofilenamecheck ]]; then
      echo "Please enter 'yes' or 'no' as the argument for -c. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
      if [[ $nofilenamecheck != [Yy]* && $nofilenamecheck != [Nn]* ]]; then
         echo "'yes' or 'no' should be provided after -c in syntax, other answer is not valid"
         exit 2
      fi 
   fi
fi
...
then
  ...
  echo "no input for Standby Unique Name. We assume it is the same as the Standby Instance Name"
  uniquename=$toraclesid
fi
...
echo "The following Oracle parameters are recommended to be in ${dbpfile}. 
The following are just the examples. The exact values may depend on your environment.
...
*.FAL_SERVER='$sdbname'
*.LOG_ARCHIVE_CONFIG='DG_CONFIG=($sdbname,$uniquename)'
...
if [[ $noresume = [Yy]* ]]; then
   if [[ $nofilenamecheck = [Yy]* ]]; then
      echo "DUPLICATE DATABASE FOR STANDBY BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
   else
      echo "DUPLICATE DATABASE FOR STANDBY BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
   fi
else
   if [[ $nofilenamecheck = [Yy]* ]]; then
      echo "DUPLICATE DATABASE FOR STANDBY BACKUP LOCATION '$backup_location' nofilenamecheck;" >> $drmanfiled
   else
