...
if [[ ! -d "${mount}1" ]]; then
   echo "
   Directory ${mount}1 doesn't exist, no backup files. Check the arguments for -m"
   exit 1
fi
...
if [[ ! -d "${mount}1" ]]; then
   echo "Directory ${mount}1 doesn't exist, no backup files. Check the arguments for -m"
   exit 1
fi
...
Cohesity recovery view and NFS mounts need to done before rnning this script.
