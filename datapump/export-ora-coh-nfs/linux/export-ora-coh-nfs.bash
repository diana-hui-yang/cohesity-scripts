...
function show_usage {
echo "usage: export-ora-coh-nfs.bash -s <Sqlplus connection> -h <host> -o <Oracle_sid> -d <PDB database> -m <mount-prefix> -n <number of mounts> -p <number of parallels> -e <retention> -z <file size> -x <ORACLE_HOME> -c <export options> -w <yes/no>"
echo " "
...
while getopts ":s:h:o:d:m:n:p:e:z:x:c:w:" opt; do
  case $opt in
    s ) sqlplusc=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
...
if [[ -n $pdbname ]]; then
  ...
  echo "export ORACLE_PDB_SID=$pdbname" >> $impdpfile
   ...
   echo "#Import database ${dbname}" >> $expdpfile
fi
