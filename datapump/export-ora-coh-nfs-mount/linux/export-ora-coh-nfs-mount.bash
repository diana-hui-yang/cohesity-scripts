...
function show_usage {
echo "usage: export-ora-coh-nfs-mount.bash -s <Sqlplus connection> -h <host> -o <Oracle_sid> -d <PDB database> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of paralles> -e <retention> -z <file size> -x <ORACLE_HOME> -w <yes/no>"
echo " "
...
while getopts ":s:h:o:d:y:v:m:n:p:e:z:x:w:" opt; do
  case $opt in
    s ) sqlplusc=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
...
#        fi
	if [[ -n $pdbname ]]; then		
	   exportpath[$j]="EXPDP_COH${i}:expdp${j}_${pdbname}_%U_${DATE_SUFFIX}.dmp"
        else
...
if [[ -n $vipfile ]]; then
   sleep 5 
#   echo "will umount Cohesity nfs mounts for this application"
   umount_coh
fi
