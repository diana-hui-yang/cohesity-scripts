...
if [[ -n $pdbname ]]; then
   if [[ -n $dba_exp_option ]]; then
       ...
       impcmd="${expcmd} LOGFILE=EXPDP_COH_LOG:dba.impdp.${DATE_SUFFIX}.log ${dba_exp_option} PARALLEL=$parallel"
   else
       ...
       impcmd="${expcmd} LOGFILE=EXPDP_COH_LOG:${pdbname}.impdp.${DATE_SUFFIX}.log FULL=Y PARALLEL=$parallel"
   fi
else
   if [[ -n $dba_exp_option ]]; then
       expcmd="${expcmd} LOGFILE=EXPDP_COH_LOG:dba.expdp.${DATE_SUFFIX}.log ${dba_exp_option} FILESIZE=${filesize}G PARALLEL=$parallel"
       ...
       impcmd="${expcmd} LOGFILE=EXPDP_COH_LOG:${dbname}.impdp.${DATE_SUFFIX}.log FULL=Y PARALLEL=$parallel"	   
   fi
fi
