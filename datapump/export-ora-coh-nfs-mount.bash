...
function show_usage {
echo "usage: export-ora-coh-nfs-mount.bash -s <Sqlplus connection> -h <host> -o <Oracle_sid> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of paralles> -e <retention> -z <file size> -x <ORACLE_HOME> -w <yes/no>"
echo " "
...
while getopts ":s:h:o:y:v:m:n:p:e:z:x:w:" opt; do
  case $opt in
    s ) sqlplusc=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
...
echo "Create directory if it is not created yet" >> $runlog
...
grep -i EXPDP_COH_LOG $dbadir 
      
if [ $? -eq 1 ]; then
  echo "Export directory expdp_coh_log has not been created yet, creating now"
  $sqllogin << EOF
  create or replace directory EXPDP_COH_LOG as '${expdplogdir}';
  exit
EOF
  if [ $? -eq 0 ]; then
     echo "dba_directories expdp_coh_log in Oracle was created" >> $runlog
     echo "dba_directories expdp_coh_log in Oracle  was created"
  else
  ...
  echo "Export directory expdp_coh_log has been created already" 
fi
...
#  echo $mount_cnt
  ...
  if [ "$mount_cnt" -eq 1 ]; then
     ...
     if [[ ! -d "${mount}${i}/exports" ]]; then
        echo "Directory ${mount}${i}/exports does not exist, create it"
        if mkdir -p ${mount}${i}/exports; then
          echo "${mount}${i}/exports is created"
        else
          echo "creating ${mount}${i}/exports failed. There is a permission issue"
          exit 1
        fi
     fi
	...
	grep -i EXPDP_COH$i $dbadir
	  ...
	  $sqllogin << EOF
	  create or replace directory EXPDP_COH$i as '${mount}${i}/exports';
          exit
EOF
          if [ $? -eq 0 ]; then
	     echo "dba_directories expdp_coh$i in Oracle was created" >> $runlog
	     echo "dba_directories expdp_coh$i in Oracle  was created"
	  else
	...
	exportpath[$j]="EXPDP_COH${i}:expdp${j}_${dbname}_%U_${DATE_SUFFIX}.dmp"
     fi
