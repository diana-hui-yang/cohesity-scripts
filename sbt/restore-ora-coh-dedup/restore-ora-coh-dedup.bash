#!/bin/bash
#
# Name:         restore-ora-coh-dedup.bash
#
# Function:     This script restore Oracle database from Oracle backup 
#               using Cohesity SBT tape backup script backup-ora-coh-dedup.bash.
#
#
...
function show_usage {
echo "usage: restore-ora-coh-dedup.bash -r <RMAN login> -h <backup host> -i <Oracle instance name> -d <Oracle_DB_Name> -t <point-in-time> -l <full restore or partial restore> -j <vip file> -v <view> -s <sbt home> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -c <yes/no> -w <yes/no>"
echo " -r : RMAN login (example: \"rman target / \", optional)"
echo " -h : backup host (optional)"
...
echo " -f : yes means force. It will testore Oracle database without prompt"
echo " -c : yes means this script will catalog archivelogs before recovery" 
echo " -w : yes means preview rman backup scripts (Optional)"
echo " -x : yes means run RMAN restore validate (Optional)"
}

while getopts ":r:h:i:d:t:l:j:v:s:p:o:f:c:w:x:" opt; do
  case $opt in
    r ) rmanlogin=$OPTARG;;
    h ) host=$OPTARG;;
    i ) oraclesid=$OPTARG;;
...
#echo $oraclesid  $full $view $sbthome $vipfile

# Check required parameters
#if test $host && test $dbname && test $mount && test $numm
if test $oraclesid  && test $view && test $sbthome && test $vipfile
then
  :
else
  show_usage 
...
echo "run {" >> $spfile_rmanfile
echo "allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';" >> $spfile_rmanfile
echo "restore spfile from '$controlfile';" >> $spfile_rmanfile
echo "release CHANNEL c1;" >> $spfile_rmanfile
echo "}" >> $spfile_rmanfile

echo "will create controlfile rman backup script"
echo "run {" >> $controlfile_rmanfile
echo "allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';" >> $controlfile_rmanfile
echo "restore CONTROLFILE from '$controlfile';" >> $controlfile_rmanfile
echo "release CHANNEL c1;" >> $controlfile_rmanfile
echo "}" >> $controlfile_rmanfile
...
#           echo "return is $return"
	    ...
	    echo "IP $ip can be connected"
	       ...
	       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
...
echo "}" >> $restore_rmanfile
echo "exit;" >> $restore_rmanfile
...
echo "alter database open resetlogs;" >> $recover_rmanfile
echo "exit;" >> $recover_rmanfile
...
if [[ $validate = "yes" || $validate = "Yes" || $validate = "YES" ]]; then
   create_rman_restore_database_file
