...
function show_usage {
echo "usage: restore-ora-coh-dedup.bash -r <RMAN login> -h <backup host> -i <Oracle instance name> -d <Oracle_DB_Name> -t <point-in-time> -l <full restore or partial restore> -j <vip file> -v <view> -s <sbt home> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -w <yes/no>"
...
echo "
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)';
...
#           echo "return is $return"
	    ...
	    echo "IP $ip can be connected"
	       ...
	       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)';"
