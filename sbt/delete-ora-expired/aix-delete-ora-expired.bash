...
function show_usage {
echo "usage: aix-delete-ora-expired.bash -h <host> -a <yes/no> -o <Oracle_DB_Name> -d <Backup Files Directory> -r <specified day> -f <yes/no>" 
...
while getopts ":h:a:o:d:r:f:" opt; do
  case $opt in
    h ) host=$OPTARG;;
	a ) all=$OPTARG;;
    o ) dbname=$OPTARG;;
...
if [[ $all = "all" || $all = "all" || $all = "all" || $all = "yes" || $all = "Yes" || $all = "YES" ]]; then
  ...
  then
    ...
    echo "Oracle_DB_Name is not provided"
    echo " "
    show_usage 
    exit 1
  fi
...
if [[ $allfile = "yes" ]]; then
   ...
   runlog=$DIR/log/$host/delete-ora-expired.${dbname}.$DATE_SUFFIX.log
   filelist=$DIR/log/$host/files.${dbname}.$DATE_SUFFIX
fi
...
function listfiles {
   ...
   if [[ $allfile = "yes" ]]; then
      ...
      /opt/freeware/bin/find $backupdir -type f -mtime +$ret |  grep -v "snapshot" > $filelist

   else
      echo ${dbname} "backup files older than $ret days are listed below"
	  echo ${dbname} "backup files older than $ret days are listed below" >> $runlog
      echo "  "
      echo "--------------------"

      /opt/freeware/bin/find $backupdir -type f -mtime +$ret -name ${dbname}_* |  grep -v "snapshot" > $filelist
...
function deletefiles {
   ...
   if [[ $allfile = "yes" ]]; then
      ...
      echo "delete " ${dbname} "backup files older than $ret days"
	  echo "delete " ${dbname} "backup files older than $ret days" >> $runlog
      echo "  "
      echo "--------------------"
   ...
   /opt/freeware/bin/find $backkupdir -type d -empty -delete
}
