...
while getopts ":r:c:h:n:o:a:i:y:f:v:s:p:e:l:b:z:m:t:g:j:d:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
	...
	if [[ -n $ip ]]; then
	      ...
	      if [[ $encryption = [Yy]* ]]; then
		    ...
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
	            ...
	            elif [[ $sdedup = [Yy]* && $rpctype != [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		   fi
		    ...
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    fi
		 else
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
		       ...
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
		    fi
		 fi
	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $rpctype != [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
                       ...
                       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
            	    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then 
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.bdf';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $rpctype != [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.bdf';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U.bdf';" 
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U.blf';"
		    fi
	  	 fi
	      ...
	      if [[ $j -lt $parallel ]]; then
		    ...
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.bdf';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $rpctype != [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)' format './$host/$dbname/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $rpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.bdf';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $rpctype != [Yy]* ]]; then
		       ...
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1)' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $rpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U.bdf';"
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_max_inflight_chunking_requests,gflag-value=-1,gflag-name=sbt_use_grpc,gflag-value=false)' format './$host/$dbname/%d_%T_%U.blf';"
		    fi
		 fi
	         unallocate[j]="release channel c$j;"
              fi
