	   ...
	   if [[ -n $racconns ]]; then
		 ...
		 if [[ $encryption = [Yy]* ]]; then
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
            	    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
            	    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then 
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    fi    
		 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	      ...
	      if [[ $j -lt $parallel ]]; then
		    ...
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
	               allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrycert})';"
   	               allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup = [Yy]* && $grpctype != [Yy]* ]]; then
		       allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})';"
                       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype = [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    elif [[ $sdedup != [Yy]* && $grpctype != [Yy]* ]]; then
		       ...
		       allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,disable_source_side_dedup=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrycert})' format './$host/$dbname/%d_%T_%U.blf';"
		    fi
         	 else
		    if [[ $sdedup = [Yy]* && $grpctype = [Yy]* ]]; then
