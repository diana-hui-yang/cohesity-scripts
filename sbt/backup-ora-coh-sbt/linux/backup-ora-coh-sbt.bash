...
if [[ $level = "offline" || $level = "full" ]]; then
   if [[ -z $sectionsize ]]; then
      echo "backup TAG '$TAG' database filesperset 1 format './$host/$db_name/%d_%T_%U_full.bdf';" >> $rmanfiled
   else
      echo "backup TAG '$TAG' database section size ${sectionsize}G filesperset 1 format './$host/$db_name/%d_%T_%U_full.bdf';" >> $rmanfiled
   fi
else
   if [[ -z $sectionsize ]]; then
