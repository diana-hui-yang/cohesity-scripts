...
#           echo "return is $return"
	      ...
	      if [[ $j -eq 1 ]]; then
		     if [[ -n $cohesityname ]]; then
	            echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$cohesityname:/$view,vips=$cohesityname,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
			 else
			    echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U';" >> $rmanfiled
			 fi
#	         echo "Delete obsolete;" >> $rmanfiled
	      ...
	      if [[ -n $racconns ]]; then
	            ...
	            allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.bdf';"
                    allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.blf';"
		 ...
		 if [[ $j -lt $parallel ]]; then
	           allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.bdf';"
   	           allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'BLKSIZE=1048576,SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)' format './$host/$dbname/%d_%T_%U.blf';"
	           unallocate[j]="release channel c$j;"
                 fi
                 j=$[$j+1]
 	      fi
