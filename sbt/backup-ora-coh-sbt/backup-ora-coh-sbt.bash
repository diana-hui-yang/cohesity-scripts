...
function show_usage {
echo "usage: backup-ora-coh-sbt.bash -r <RMAN login> -h <host> -o <Oracle_DB_Name> -a <archive only> -i <incremental level> -f <vip file> -v <view> -s <sbt home> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -w <yes/no>"
    ...
    if [[ $oracle_sid == ${dbname} ]]; then
       ...
       if [[ $lastc =~ ^[0-9]+$ ]]; then
	    ...
	    if [[ -z $hostdefinded ]]; then
   	      echo "This is RAC environment, scanname should be provided after -r option"
	      echo "  "
	      exit 2
   	    else
              echo "Oracle database $dbname is up on $host. Backup can start"
