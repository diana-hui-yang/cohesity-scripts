...
# 10/03/20 Diana Yang   Add option to use cluster DNS name
#
#################################################################

function show_usage {
echo "usage: backup-ora-coh-sbt.bash -r <RMAN login> -h <host> -o <Oracle_DB_Name> -a <archive only> -i <incremental level> -c <Cohesity Cluster DNS Name> -f <vip file> -v <view> -s <sbt home> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -m <ORACLE_HOME> -w <yes/no>"
echo " "
...
echo " -c : Cohesity Cluster DNS name. It precedes Cohesity VIPs"
echo " -f : The file lists Cohesity Cluster VIPs. This file is NOT used if Cohesity Cluster name is provided"
...
echo " -m : ORACLE_HOME (provide ORACLE_HOME if the database is not in /etc/oratab. Otherwise, it is optional.)"
echo " -z : section size in GB (Optional, default is no section size)"
echo " -w : yes means print rman backup scripts only. The RMAN script is not executed"
echo "
...
while getopts ":r:h:o:a:i:c:f:v:s:p:e:l:z:m:w:" opt; do
  case $opt in
    r ) rmanlogin=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
...
echo ${rmanlogin}, $retday, $arch, $dbname, $clustername, $vipfile, $host, $view
...
if test $clustername; then
   echo "Cohesity Cluster is $clustername"
else
   echo "Cohesity Cluster DNS name is not provided, we will use VIPs in $vipfile"
...
if [[ -n $clustername ]]; then
  echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$clustername:/$view,vips=$clustername,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U';
CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM $parallel BACKUP TYPE TO BACKUPSET;" >> $rmanfiled
...
#	         echo "Delete obsolete;" >> $rmanfiled
	        ...
	        allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)';"
   	        allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)';"
...
if [[ -n $clustername ]]; then
  echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$clustername:/$view,vips=$clustername,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U';
CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM $parallel BACKUP TYPE TO BACKUPSET;" >> $rmanfilear
else
  echo "RUN {
  " > $rmanfilear
