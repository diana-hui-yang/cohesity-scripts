#!/bin/bash
#
# Name:         aix-backup-ora-coh-sbt.multi.bash
#
...
# 01/22/21 Diana Yang   Add up to four threads per channel to increase performance
#
#################################################################

function show_usage {
echo "usage: aix-backup-ora-coh-sbt.multi.bash -r <Target connection> -c <Catalog connection> -h <host> -n <rac-node1-conn,rac-node2-conn,...> -o <Oracle_DB_Name> -a <archive only> -i <incremental level> -y <Cohesity-cluster> -f <vip file> -v <view> -s <sbt home> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -m <ORACLE_HOME> -w <yes/no> -b <number of archive logs>"
echo " "
...
while getopts ":r:c:h:n:o:a:i:y:f:v:s:p:e:l:b:z:m:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
if [[ -z $archcopynum  ]]; then
  echo "Only backup archive logs once"
  archcopynum=1
else
  echo "backup archive logs $archcopynum"
fi
...
#     echo "return is $return"
      if echo $return | grep -i "0 received"; then
	 ...
	 if [[ $j -gt 0 && $j -lt $viptotal ]]; then
	    vips=$vips:$ip
         elif [[ $j -eq $viptotal ]]; then 
	    break
	 fi
         j=$[$j+1] 	      
      fi
...
echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$firstvip:/$view,vips=$vips,gflag-name=sbt_use_grpc,gflag-value=true)';" >> $rmanfiled
...
while [ $j -lt $parallel ]; do
   ...
   if [[ -n $racconns ]]; then
      if [[ $j -lt $parallel ]]; then
	     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$firstvip:/$view,vips=$vips,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U.bdf';"
         allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$firstvip:/$view,vips=$vips,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U.blf';"
	  ...
	  if [[ $j -lt $parallel ]]; then
	     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$firstvip:/$view,vips=$vips,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U.bdf';"
   	     allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$firstvip:/$view,vips=$vips,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U.blf';"
	     unallocate[j]="release channel c$j;"
      fi
      j=$[$j+1]
   fi
...
#   echo "backup archivelog all filesperset 1 not backed up $archcopynum times delete input;" >> $rmanfilea
   echo "backup archivelog all not backed up $archcopynum times delete input;" >> $rmanfilea
else
#   echo "backup archivelog all filesperset 1 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilea
...
if [[ $archretday -eq 0 ]]; then
#   echo "backup archivelog all filesperset 1 not backed up $archcopynum times delete input;" >> $rmanfilear
   echo "backup archivelog all not backed up $archcopynum times delete input;" >> $rmanfilear
else
#   echo "backup archivelog all filesperset 1 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilear
