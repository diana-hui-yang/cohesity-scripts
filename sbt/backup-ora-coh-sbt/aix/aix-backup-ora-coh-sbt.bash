...
#	  echo "Delete obsolete;" >> $rmanfiled
	  ...
	  echo "RUN {" >> $rmanfilea
       ...
       if [[ -n $racconns ]]; then
          if [[ $j -lt $parallel ]]; then
             ...
             allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' connect='$cred@${arrconns[$k]}' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
	  ...
	  if [[ $j -lt $parallel ]]; then
	     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
   	     allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip)';"
	     unallocate[j]="release channel c$j;"
          fi
          j=$[$j+1]
       fi
...
#   echo "backup archivelog all filesperset 8 not backed up $archcopynum times delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilea
   echo "backup archivelog all not backed up $archcopynum times delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilea
else
#   echo "backup archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilea
   echo "backup archivelog all not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilea
fi
...
#   echo "backup archivelog all filesperset 8 not backed up $archcopynum times delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilear
   echo "backup archivelog all not backed up $archcopynum times delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilear
else
#   echo "backup archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilear
   echo "backup archivelog all not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input format './$host/$dbname/%d_%T_%U.blf';" >> $rmanfilear
fi
