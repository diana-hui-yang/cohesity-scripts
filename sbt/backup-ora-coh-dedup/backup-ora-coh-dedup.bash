...
#           echo "return is $return"
	      ...
	      if [[ $i -eq 1 ]]; then
	         echo "CONFIGURE CHANNEL DEVICE TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)';" >> $rmanfiled
#	         echo "Delete obsolete;" >> $rmanfiled
          ...
          if [[ $j -lt $parallel ]]; then
		     allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d__%T_%U.bdf';"
			 allocate_archive[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbthome/libsbt_6_and_7_linux-x86_64.so,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag-name=sbt_use_grpc,gflag-value=true)' format '%d_%T_%U.blf';"
