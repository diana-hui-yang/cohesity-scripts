...
# 07/22/20 Diana Yang   Backup database remotely
#
#################################################################

function show_usage {
echo "usage: backup-ora-coh-dedup.bash -r <RMAN login> -h <host> -o <Oracle_DB_Name> -a <archive only> -i <incremental level> -f <vip file> -v <view> -s <sbt home> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size>"
...
while getopts ":r:h:o:a:i:f:v:s:p:e:l:z:" opt; do
  case $opt in
    r ) rmanlogin=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
...
echo "CONFIGURE CONTROLFILE AUTOBACKUP OFF;" >> $rmanfilear
echo "   " >> $rmanfilear
echo "RUN {" >> $rmanfilear
