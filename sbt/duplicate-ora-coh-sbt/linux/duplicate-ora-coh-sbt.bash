...
echo "usage: duplicate-ora-coh-sbt.bash -r <Source ORACLE connection> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -n <new PDB name> -a <Auxiliary database> -f <yes/no> -m <noresume> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
while getopts ":r:c:h:d:i:y:b:t:e:j:v:s:p:o:u:n:a:f:m:g:k:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
...
if [[ -n $pdbcopyset ]]; then
   ...
   if [[ -z $pdbcopy ]]; then
      echo "Please enter 'yes' or 'no' as the argument for -l. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
      if [[ $pdbcopy != [Yy]* && $pdbcopy != [Nn]* ]]; then
         echo "'yes' or 'no' should be provided after -l in syntax, other answer is not valid"
	 exit 2
      fi 
   fi
fi
...
if [[ -z ${backupdir} ]]; then
   ...
   if [[ -z ${backupdir} ]]; then
      echo "The directory ${shost}/${sdbname} or ${shostshort}/${sdbname} doesn't exist in view ${view} of this Cohesity Cluster. Please check the input"
      exit 2
   fi
else
...
if [[ ! -f ${dbpfile} ]]; then
    echo "The oracle pfile $dbpfile doesn't exist. Please check the instance name or create the pfile first."
    exit 2
fi
...
if [ $? -eq 0 ]; then
   echo "Unpluggin pdb database from database $adbname failed"
...
#   else
#      $sqllogin << EOF > $pluglog
#         create pluggable database $npdbname as clone using '$pdbplug' FILE_NAME_CONVERT = ('$adbname','$toraclesid') copy;
#         alter pluggable database $npdbname open;
#EOF
#   fi
else
...
#   else
#      $sqllogin << EOF > $pluglog
#         create pluggable database $pdbname as clone using '$pdbplug' FILE_NAME_CONVERT = ('$adbname','$toraclesid') copy;
#         alter pluggable database $pdbname open;
#EOF
#   fi 
fi
...
if [ $? -eq 0 ]; then
   echo "Pluggin pdb database to database $toraclesid failed"
