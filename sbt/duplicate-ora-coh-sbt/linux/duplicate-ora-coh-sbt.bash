...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -d <Source Oracle database> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
if [[ `grep -i error ${drmanlog}` ]]; then
  if [[ `grep -i "c0 not allocated" $drmanlog` ]]; then
     ...
     echo "Check rmanlog file is $drmanlog"
