...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -d <Source Oracle database> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -f <yes/no> -m <noresume> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
while getopts ":r:c:h:d:i:y:b:t:e:j:v:s:p:o:u:f:m:g:k:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
...
if [[ -z ${backupdir} ]]; then
   echo "The directory ${shost}/${sdbname} or ${shost}/${sdbname^^} doesn't exist in view ${view} of this Cohesity Cluster. Please check the input"
   exit 2
fi
