...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <Target connection> -e <Catalog connection> -b <Source host> -t <Source Oracle database> -a <Target host> -d <Target Oracle_DB_Name> -y <Cohesity-cluster> -l <file contain duplicate settting>  -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -c <source PDB> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
echo " -t : Source Oracle instance name (database backup was taken). If it is not RAC, it is the same as DB name. If it is RAC, it is the instance name like cohcdba2"
echo " -d : Target Oracle_DB_Name (Oracle duplicate database)"
...
while getopts ":r:e:b:a:d:t:y:l:j:v:s:p:o:c:f:g:k:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    e ) catalogc=$OPTARG;;
    b ) shost=$OPTARG;;
...
echo $shost, $sdbname, $toraclesid, $view, $vipfile, $preview, $ora_pfile
echo "  "
...
while [ $j -lt $parallel ]; do
   while IFS= read -r ip; do
      ...
      if [[ -n $ip ]]; then
	    ...
	    if [[ $encryption = [Yy]* ]]; then
	       if [[ $grpctype = [Yy]* ]]; then
                  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrydir}/cert.cfg)';"
               else
		  allocate_database[$j]="allocate auxiliary CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)';"
	       fi
	    else
	       if [[ $grpctype = [Yy]* ]]; then
