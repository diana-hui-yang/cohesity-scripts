...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -e <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -n <new PDB name> -l <yes/no> -a <Auxiliary database> -f <yes/no> -m <noresume> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
echo " -a : Auxiliary database to restore pluggable database"
...
while getopts ":r:c:h:d:i:y:b:t:e:j:v:s:p:o:u:n:l:a:f:m:g:k:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
...
plugsql=$DIR/log/$thost/$toraclesid.plug.$DATE_SUFFIX.sql
...
EOF
   ...
   if [[ $force != [Yy]* ]]; then
      if grep -i $pdbguid $stdout > /dev/null && grep -i $adbname $stdout > /dev/null ; then
         echo "The target database $toraclesid has directory structure using \"GUID\" of the pdb name $pdbname that needs to be duplicated and \"auxiliary database name\" $adbname"
...
$ssqllogin << EOF > /dev/null
   spool $stdout replace
   select SEQUENCE#,FIRST_TIME from v\$backup_redolog where FIRST_TIME > sysdate-7 order by FIRST_TIME desc;
EOF
      ...
      echo "The last backed up archive log sequence is $sequence"
   fi
fi
...
if [[ -n $sequence ]]; then
echo "Oracle recovery point-in-time is define. Set recover time until $sequence"
...
echo "unplug pdb database in CDB database $ORACLE_SID"
...
if [ $? -eq 0 ]; then
   echo "Error: Unpluggin pdb database from database $adbname failed"
   echo "Need to check the error before proceeding "
   exit 1
else
   echo "The plug of pdb database to database $toraclesid is successful. Destroy the auxiliary database $adbname."
fi

echo "create new pdb database on target database $toraclesid"
export ORACLE_SID=$toraclesid
if [[ -n $npdbname ]]; then
   if [[ $pdbcopy != [Yy]* ]]; then
...
EOF
         ...
         echo "create pluggable database $npdbname as clone using '$pdbplug' FILE_NAME_CONVERT = ('$adbname','$toraclesid') copy;" >> $plugsql
         echo "alter pluggable database $npdbname open;" >> $plugsql
         echo "exit"  >> $plugsql
         echo "Plugging database $npdbname sql commands are created in $plugsql file. Please modify FILE_NAME_CONVERT content according to your own enviroment and run it manually"
   fi
else
   if [[ $pdbcopy != [Yy]* ]]; then
      $sqllogin << EOF > $pluglog
...
EOF
         ...
         echo "create pluggable database $pdbname as clone using '$pdbplug' FILE_NAME_CONVERT = ('$adbname','$toraclesid') copy;" >> $plugsql
         echo "alter pluggable database $pdbname open;" >> $plugsql
         echo "exit"  >> $plugsql
         echo "Plugging database $pdname sql commands are created in $plugsql file. Please modify FILE_NAME_CONVERT content according to your own enviroment and run it manually"
   fi 
fi
...
if [[ $pdbcopy != [Yy]* ]]; then
   ...
   if [ $? -eq 0 ]; then
      echo "Error: Pluggin pdb database to database $toraclesid failed"
      echo "Will not delete auxiliary database $adbname."
      exit 1
   else
      echo "Pluggin pdb database to database $toraclesid is successful. Destroy the auxiliary database $adbname."
   fi
fi

}

function destroy_aux {
...
if [[ $preview != [Yy]* ]]; then
   ...
   if [[ $force = [Yy]* ]]; then
      ...
      echo "The init file $dbpfile needs to be ccreated first. The successful of this duplication depends on the content of that file.
	       "
      if [[ -z $pdbname ]]; then
