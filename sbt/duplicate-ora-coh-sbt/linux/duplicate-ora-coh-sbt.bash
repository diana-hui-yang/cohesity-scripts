...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <Target connection> -h <backup host> -c <Catalog connection> -i <Oracle instance name> -d <Source Oracle database> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -t <sequence> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -f <yes/no> -w <yes/no> -g <yes/no> -k <cert path> -x <yes/no>" 
echo " "
...
while getopts ":r:c:h:d:i:y:b:t:e:j:v:s:p:o:u:f:g:k:x:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
