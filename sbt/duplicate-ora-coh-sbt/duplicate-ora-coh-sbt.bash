...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <RMAN login> -b <backup host> -a <target host> -d <Source Oracle_DB_Name> -t <Target Oracle instance name> -f <file contain duplicate settting> -i <file contain setting to new spfile> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -c <source PDB> -e <target PDB> -w <yes/no>" 
...
echo " -e : Target pluggable database (if this input is empty, it is standardalone or CDB database restore)"
echo " -w : yes means preview rman duplicate scripts"
}

while getopts ":r:b:a:d:t:f:i:j:v:s:p:o:c:e:w:" opt; do
  case $opt in
    r ) rmanlogin=$OPTARG;;
    b ) shost=$OPTARG;;
    a ) thost=$OPTARG;;
...
#   echo ora_pfile is ${ora_pfile}
   ...
   if test $spdbname; then
      if test $tpdbname; then
	    :
	  else
	    echo "target pdb name needs to be defined and should be different from source pdb name"
		exit 1
	  fi
   fi
fi
...
if [[ ! -z $ora_spfile ]]; then
  if test -f $ora_spfile; then
    if [[ -z $spdbname ]]; then
	   ...
	   if [[ -z $tpdbname ]]; then
	     ...
	     echo "duplicate database $sdbname to $toraclesid pluggable database $spdbname as $tpdbname" >> $drmanfiled
	   fi
    fi
  ...
  if [[ -z $spdbname ]]; then
     ...
     if [[ -z $tpdbname ]]; then
	   ...
	   echo "duplicate database $sdbname to $toraclesid pluggable database $spdbname as $tpdbname nofilenamecheck;" >> $drmanfiled
	 fi
  fi
fi  
