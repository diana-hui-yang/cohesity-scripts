...
function show_usage {
echo "usage: duplicate-ora-coh-sbt.bash -r <RMAN login> -b <backup host> -a <target host> -d <Source Oracle_DB_Name> -t <Target Oracle instance name> -f <file contain duplicate settting> -i <file contain setting to new spfile> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -c <pluggable database> -w <yes/no>" 
...
echo $rmanlogin, $shost, $sdbname, $tdbname, $view, $vipfile
echo "  "
echo "  "

# Check required parameters
#if test $shost && test $sdbname && test $tdbname && test $tdbdir && test $mount && test $num
#if test $rmanlogin && test $shost && test $sdbname && test $tdbname && test $view && test $vipfile
if test $shost && test $sdbname && test $tdbname && test $view && test $vipfile
then
  :
else
  show_usage 
...
if [[ ! -z $ora_spfile ]]; then
  if test -f $ora_spfile; then
    if [[ -z $pdbname ]]; then
       echo "duplicate database $sdbname to $tdbname" >> $drmanfiled
    else
       echo "duplicate database $sdbname to $tdbname pluggable database $pdbname" >> $drmanfiled
    fi
  ...
  if [[ -z $pdbname ]]; then
     echo "duplicate database $sdbname to $tdbname nofilenamecheck;" >> $drmanfiled
  else
     echo "duplicate database $sdbname to $tdbname pluggable database $pdbname nofilenamecheck;" >> $drmanfiled
  fi
fi  
