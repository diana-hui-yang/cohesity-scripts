...
# 08/06/20 Diana Yang   Rename this file from restore-ora-coh-dedup.bash to aix-restore-ora-coh-sbt.bash
...
then
  ...
  echo "Oracle instance name is provided. If it is RAC or Standby database, please provide Oracle database name" 
  dbname=$oraclesid
fi
...
if [[ ! -s ${controlfile_list} ]]; then
   ${DIR}/tools/sbt_list --view=$view/$host/$oraclesid --vips=$ip | sort -nk1 | grep -i ${dbname}_c- > ${controlfile_list}
   backupdir=${host}/${oraclesid}
else
   backupdir=${host}/${dbname}
fi
