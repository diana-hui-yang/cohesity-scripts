...
EOF
   ...
   if [[ $force = [Yy]* ]]; then
     if [[ $full != [Yy]* ]]; then
       read -p "The restore will overwrite the database $dbname. Type YES to confirm or any other keys to cancel: " answer3
     else
       answer3=yes
     fi
   else
...
if [[ $full = [Yy]* ]]; then
   ...
   read -p "The database $dbname will be overwritten. Type YES to confirm or any other keys to cancel: " answer2
   if [[ $answer2 = [Yy]* ]]; then
      if [[ -n $catalogc ]]; then
         create_rman_restore_controlfile_catalog
      else
