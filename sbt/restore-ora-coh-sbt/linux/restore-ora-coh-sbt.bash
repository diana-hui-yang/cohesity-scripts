...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrydir}/cert.cfg)';
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true)';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false)';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrydir}/cert.cfg)';
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)';
restore spfile FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
" >> $controlfile_rmanfile
      ...
      echo "
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)';
restore CONTROLFILE FROM AUTOBACKUP;
release CHANNEL c1;
}
" >> $controlfile_rmanfile
...
while [ $j -lt $parallel ]; do
   ...
   while IFS= read -r ip; do
      ...
      if [[ -n $ip ]]; then
	    ...
	    if [[ $encryption = [Yy]* ]]; then
	       if [[ $grpctype = [Yy]* ]]; then
	    	  allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrydir}/cert.cfg)';"
	       else
	          allocate_database[$j]="allocate CHANNEL c$j TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${encrydir}/cert.cfg)';"
	       fi
            else
	       if [[ $grpctype = [Yy]* ]]; then
