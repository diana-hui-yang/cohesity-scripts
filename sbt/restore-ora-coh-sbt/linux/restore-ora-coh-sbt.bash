...
function show_usage {
echo "usage: restore-ora-coh-sbt.bash -h <backup host> -c <Catalog connection> -i <Oracle instance name> -d <Oracle_DB_Name> -y <Cohesity-cluster> -b <file contain restore settting> -t <point-in-time> -l <yes/no> -j <vip file> -v <view> -s <sbt file name> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -w <yes/no>"
echo " "
...
if [ ! -f $sbtname ]; then
#check whether sbt library is in ${DIR}/lib/libsbt_linux-x86_64.so
   sbtname=${DIR}/lib/libsbt_linux-x86_64.so
   if [ ! -f $sbtname ]; then
