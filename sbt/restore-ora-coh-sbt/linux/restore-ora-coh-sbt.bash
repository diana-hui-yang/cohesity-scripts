...
then
  ...
  echo "Oracle instance name is provided, but not Oracle database name. We assume this is not a RAC database" 
  dbname=$oraclesid
fi
...
if [[ -n $grpctypeset ]]; then
   if [[ -z $grpctype ]]; then
      echo "yes should be provided after -x in syntax. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
...
if [[ -n $encryptionset ]]; then
   if [[ -z $encryption ]]; then
      echo "yes should be provided after -g in syntax. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
...
if [[ -n $encrydirset ]]; then
   if [[ -z $encrydir ]]; then
      echo "A file should be provided after -k in syntax. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
      if [[ $encrydir != / ]]; then
...
if [[ -n $itimeset ]]; then
   if [[ -z $itime ]]; then
      echo "yes should be provided after -t in syntax. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
      if [[ $itime == *\"* ]]; then
...
#echo $ip
${DIR}/tools/sbt_list --view=$view/$host/$dbname --vips=$ip | sort -nk1 | grep -i ${dbname}_c- > ${controlfile_list}
#echo "${DIR}/tools/sbt_list --view=$view/$host/$dbname --vips=$ip | sort -nk1 | grep ${dbname^^}_c-"
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${encrycert})';
...
run {
set dbid $dbid;
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,gflag-name=sbt_use_grpc,gflag-value=false,sbt_certificate_file=${cert})';
restore spfile from '$controlfile';
release CHANNEL c1;
}
" >> $spfile_rmanfile
...
if [[ $encryption = [Yy]* ]]; then
   if [[ $grpctype = [Yy]* ]]; then
      echo "
run {
allocate CHANNEL c1 TYPE 'SBT_TAPE' PARMS 'SBT_LIBRARY=$sbtname,SBT_PARMS=(mount_path=$ip:/$view/$host/$dbname,vips=$ip,gflag_name=use_fixed_dedup_chunking,gflag_value=true,sbt_certificate_file=${cert})';
...
if [[ $full = [Yy]* ]]; then
   ...
   read -p "Are you sure you are ready to restore database $dbname? " answer2
   if [[ $answer2 = [Yy]* ]]; then
      if [[ -n $catalogc ]]; then
         create_rman_restore_controlfile_catalog
      else
