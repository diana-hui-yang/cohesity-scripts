...
EOF
   ...
   if [[ $force = [Yy]* ]]; then
     if [[ $full != [Yy]* ]]; then
       read -p "Should we start to do restore? The restore will overwrite the database $dbname" answer3
     else
       answer3=yes
     fi
   else
...
if [[ $full = [Yy]* ]]; then
   ...
   read -p "Are you sure you are ready to restore database $dbname?. The database $dbname will be overwritten. " answer2
   if [[ $answer2 = [Yy]* ]]; then
      if [[ -n $catalogc ]]; then
         create_rman_restore_controlfile_catalog
      else
