...
while [ $j -le $parallel ]; do
	...
	echo "Check whether IP $line can be connected"
	echo "Check whether IP $line can be connected" >> $runlog
	ping $line -c 2 | grep -i error
	
	if [ $? -ne 0 ]; then
	   echo "IP $line can be connected"
	   echo "IP $line can be connected" >> $runlog
	   if [[ $i -eq 1 ]]; then
