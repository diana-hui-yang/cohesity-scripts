...
# 09/21/22 Diana Yang	change "df -h" to "df -g"
# 09/21/22 Diana Yang	change mount option to AIX (llock)
# 08/14/23 Diana Yang   Add db2profile option
#
#################################################################

function show_usage {
echo "usage: aix-backup-db2-coh-nfs-mount.bash -d <database name> -t <full or incremental> -l <offline or online> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of sessions> -e <retention> -f <DB2 profile> -w <yes/no>"
echo " "
...
db2user=`id |  /opt/freeware/bin/gawk -P '{print $1}' |  /opt/freeware/bin/gawk -F "(" -P '{ print $2 }' |  /opt/freeware/bin/gawk -F ")" -P '{ print $1}'`
db2group=`id |  /opt/freeware/bin/gawk -P '{print $2}' |  /opt/freeware/bin/gawk -F "(" -P '{ print $2 }' |  /opt/freeware/bin/gawk -F ")" -P '{ print $1}'`
