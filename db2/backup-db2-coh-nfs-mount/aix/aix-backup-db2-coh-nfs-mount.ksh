#!/bin/ksh
#
# Name:         aix-backup-db2-coh-nfs-mount.ksh
#
...
# 07/30/25 Diana Yang   Add user group permission support and error checking
#
#
#################################################################
...
umask 007
function show_usage {
echo "usage: aix-backup-db2-coh-nfs-mount.ksh -d <database name> -t <full or incremental or offline> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of sessions> -e <retention> -f <DB2 profile> -w <yes/no>"
echo " "
...
while getopts ":d:t:f:y:v:m:n:e:p:w:" opt; do
  case $opt in
    d ) dbname=$OPTARG;;
    t ) backuptype=$OPTARG;;
...
db2hist_cmd=$DIR/log/$dbname/$dbname.hist.$DATE_SUFFIX.ksh
...
db2user=`id |  awk '{print $1}' |  awk -F "(" '{ print $2 }' |  awk -F ")" '{ print $1}'`
db2group=`id |  awk '{print $2}' |  awk -F "(" '{ print $2 }' |  awk -F ")" '{ print $1}'`
...
if [ $? -ne 0 ]; then
  echo "
  DB2 list history failed at " `/bin/date '+%Y%m%d%H%M%S'`
  echo "
  DB2 list history failed at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
  exit 1
else
  echo "
  DB2 list history finished at " `/bin/date '+%Y%m%d%H%M%S'`
  echo "
  DDB2 list history finished at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog 
fi
