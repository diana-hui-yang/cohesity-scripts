...
rmanfiled_m=$DIR/log/$host/$dbname.rman_m.$DATE_SUFFIX.rcv
...
#          echo "crosscheck datafilecopy like '${mount}$i/$host/$dbname/datafile/%';" >> $rmanfiled_m
#       fi
...
if [[ $ttype = "full" || $type = "Full" || $ttype = "FULL" ]]; then
   ...
   echo "crosscheck datafilecopy all;" >> $rmanfiled_m
   echo "delete noprompt expired datafilecopy all;" >> $rmanfiled_m
   echo "crosscheck backup;" >> $rmanfiled_m
   echo "delete noprompt expired backup;" >> $rmanfiled_m
   echo "delete noprompt copy of database tag 'incre_update';" >> $rmanfiled_m
   echo "backup incremental level 1 for recover of copy with tag 'incre_update' database;" >> $rmanfiled_m
   echo "recover copy of database with tag  'incre_update';" >> $rmanfiled_m
   ...
   echo "backup type entered is not correct. It should be full or incre" >> $rmanfiled_m
   exit 1
fi
   
echo "sql 'alter system switch logfile';" >> $rmanfiled_m
...
cat $rmanfiled_b $rmanfiled_m $rmanfiled_e > $rmanfiled
