#!/bin/bash
#
# Name:         aix-list-del-ora-agedfile.bash
#
...
# 04/20/21 Diana Yang   Change search pattern
#
#################################################################


function show_usage {
echo "usage: aix-list-del-ora-agedfile.bash -h <host> -a <yes/no> -s <search pattern> -d <Backup Files Directory> -r <specified day> -f <yes/no>" 
...
function listfiles {
   ...
   if [[ $allfile = "yes" ]]; then
      ...
      echo "  "
      echo "--------------------"
      echo find $backupdir -type f -mtime +$ret -name *${match}* |  grep -v "snapshot"
      /opt/freeware/bin/find $backupdir -type f -mtime +$ret -name *${match}* |  grep -v "snapshot" > $filelist
