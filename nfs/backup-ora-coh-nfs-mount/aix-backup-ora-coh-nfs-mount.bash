...
if test $host
then
  hostde/opt/freeware/bin/finded=yes
else
  host=`hostname -s`
fi
...
if [[ -n $view ]]; then
   if [[ -z $cohesityname ]]; then
      ...
      if [[ ! -f $DIR/config/vip-list ]]; then
        echo "can't /opt/freeware/bin/find $DIR/config/vip-list file. Please provide cohesity name or populate $DIR/config/vip-list with Cohesity VIPs"
        echo " "
        show_usage
        exit 1
      fi
  ...
  do
    ...
    if [[ $oracle_sid == ${dbname} ]]; then
       ...
       if [[ $lastc =~ ^[0-9]+$ ]]; then
	    ...
	    if [[ -z $hostde/opt/freeware/bin/finded ]]; then
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    /opt/freeware/bin/find ${mount}1/$host/$dbname -type f -mtime +$retnewday -exec /bin/rm -f {} \;
    /opt/freeware/bin/find ${mount}1/$host/$dbname -depth -type d -empty -exec rmdir {} \;
  fi
