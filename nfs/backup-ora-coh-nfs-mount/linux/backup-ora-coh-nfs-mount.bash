#!/bin/bash
#
# Name:         backup-ora-coh-nfs-mount.bash
#
...
# 02/24/20 Diana Yang   get ORACLE_HOME from /etc/oratab file 
...
# 12/01/20 Diana Yang   Skip database not in archive mode
# 12/01/20 Diana Yang   Add mount/umount option
# 01/06/21 Diana Yang   Utilize vip-list file
# 01/20/21 Diana Yang   Backup pfile and spfile
...
# 10/31/22 Diana Yang   Add a RMAN compress option												  
#
#################################################################

function show_usage {
echo "usage: backup-ora-coh-nfs-mount.bash -r <Target connection> -c <Catalog connection> -h <host> -d <rac-node1-conn,rac-node2-conn,...> -o <Oracle_sid> -a <archive only> -i <incremental level> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -f <RMAN compression yes/no> -x <ORACLE_HOME> -w <yes/no> -b <number of archive logs> -t <tag>"
echo " "
...
while getopts ":r:c:h:d:o:a:i:y:v:m:n:p:e:l:b:z:k:x:t:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
if [[ -n $previewset ]]; then
   ...
   if [[ -z $preview ]]; then
      echo "Please enter 'yes' 'no' as the argument for -w. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
      if [[ $preview != [Yy]* ]]; then
...
if [[ -n $compressionset ]]; then
   if [[ -z $compression ]]; then
      echo "Please enter 'yes' or 'no' as the argument for -k. It may be empty or not all options are given an argument. All options should be given an arguments"
      exit 2
   else
...
if [[ -z $sectionsize ]]; then
   if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database filesperset 1;" >> $rmanfiled
   else	  
      echo "backup INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database filesperset 1 ;" >> $rmanfiled
   fi 
else
   if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET INCREMENTAL LEVEL $level CUMULATIVE TAG '$TAG' database section size ${sectionsize}G filesperset 1;" >> $rmanfiled
   else
...
if [[ $archretday -eq 0 ]]; then
   ...
   if [[ $compression = [Yy]* ]]; then
      echo "backup AS COMPRESSED BACKUPSET archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input filesperset 8;" >> $rmanfilea
   else
      echo "backup archivelog all filesperset 8 not backed up $archcopynum times archivelog until time 'sysdate-$archretday' delete input filesperset 8;" >> $rmanfilea
   fi 
fi
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    find ${mount}1/$host/$dbname -type f -mtime +$retnewday -exec /bin/rm -vf {} \; >> $runlog
    find ${mount}1/$host/$dbname -depth -type d -empty -exec rmdir -v {} \; >> $runlog
  fi
