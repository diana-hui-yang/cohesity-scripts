...
function show_usage {
echo "usage: backup-ora-coh-oim.bash -r <RMAN login> -h <host> -o <Oracle_DB_Name> -t <backup type> -a <archive only> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days>" 
echo " -r : RMAN login (example: \"rman target /\", optional)"
echo " -h : host (local host or scanname if it is RAC)" 
echo " -o : Oracle_DB_Name" 
...
echo "backup as copy current controlfile format '${mount}${i}/$host/$dbname/$dbname.ctl.$DATE_SUFFIX';"  >> $catalog_bash
