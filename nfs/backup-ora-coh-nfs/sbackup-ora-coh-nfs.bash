...
# 03/07/2020 Diana Yang   Rename the script to sbackup-ora-coh-nfs.bash 
# 04/07/2020 Diana Yang   Better support for RAC database
# 09/15/2020 Diana Yang   Add more options
# 10/15/2020 Diana Yang   Add Deletion of the expired backup using unix command
# 10/22/2020 Diana Yang   Add Oracle section size option
#
#################################################################

function show_usage {
echo "usage: sbackup-ora-coh-nfs.bash -r <RMAN login> -h <host> -o <Oracle_sid> -a <archive only> -i <incremental level> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -b <ORACLE_HOME> -w <yes/no>"
echo " "
...
while getopts ":r:h:o:a:i:m:n:p:e:l:z:b:w:" opt; do
  case $opt in
    r ) rmanlogin=$OPTARG;;
    h ) host=$OPTARG;;
    o ) dbname=$OPTARG;;
