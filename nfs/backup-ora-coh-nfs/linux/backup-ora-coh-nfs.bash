...
function show_usage {
echo "usage: backup-ora-coh-nfs.bash -r <Target connection> -c <Catalog connection> -h <host> -d <rac-node1-conn,rac-node2-conn,...> -o <Oracle_sid> -a <archive only> -i <incremental level> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -k <RMAN compression yes/no> -x <ORACLE_HOME> -w <yes/no> -b <number of archive logs> -t <tag>"
echo " "
...
while getopts ":r:c:h:d:o:a:i:m:n:p:e:l:b:z:k:x:t:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    find ${mount}1/$host/$dbname -type f -mtime +$retnewday -exec /bin/rm -fv {} \; >> $runlog
