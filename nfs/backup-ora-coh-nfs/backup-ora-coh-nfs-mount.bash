...
while getopts ":r:c:h:d:o:a:i:y:v:m:n:p:e:l:z:b:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
if [[ -z $cohesityname ]]; then
  ...
  vipfiletemp=${DIR}/config/${dbname}-vip-list-temp
...
#ps -efww | grep -w ${script_name}
status=`ps -efww | grep -w ${script_name} |wc -l`
echo "status=$status"
if [ "$status" -gt 3 ]; then
   echo "== "
