...
# 10/11/21 Diana Yang   Use gfind, gawk, hostname
#
#################################################################
...
if [[ $archretday -eq 0 ]]; then
   echo "backup archivelog all filesperset 8 not backed up 1 times delete input;" >> $rmanfilea
else
   echo "backup archivelog all filesperset 8 not backed up 1 times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilea
fi
...
if [[ $archretday -eq 0 ]]; then
   echo "backup archivelog all filesperset 8 not backed up 1 times delete input;" >> $rmanfilear
else
   echo "backup archivelog all filesperset 8 not backed up 1 times archivelog until time 'sysdate-$archretday' delete input;" >> $rmanfilear
fi
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    gfind ${mount}1/$host/$dbname -type f -mtime +$retnewday -exec /bin/rm -fv {} \; >> $runlog
    gfind ${mount}1/$host/$dbname -depth -type d -empty -exec rmdir -v {} \; >> $runlog
  fi
...
((timezoneoffset=`/bin/gdate -u '+%Y%m%d%H%M%S'`-`/bin/gdate '+%Y%m%d%H%M%S'`))
