#!/bin/bash
#
# Name:         aix-backup-ora-coh-nfs.bash
#
...
# 03/08/21 Diana Yang   change "awk" to "/opt/freeware/bin/gawk" 
# 03/08/21 Diana Yang   change "find" to "/opt/freeware/bin/find"
...
function show_usage {
echo "usage: aix-backup-ora-coh-nfs.bash -r <Target connection> -c <Catalog connection> -h <host> -d <rac-node1-conn,rac-node2-conn,...> -o <Oracle_sid> -a <archive only> -i <incremental level> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -x <ORACLE_HOME> -w <yes/no> -b <number of archive logs> -t <tag>"
echo " "
...
while getopts ":r:c:h:d:o:a:i:m:n:p:e:l:b:z:x:t:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
while [ $i -le $num ]; do
  ...
  mount_cnt=`df -g ${mount}$i | grep -wc "${mount}$i$"`
#  echo $mount_cnt
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    /opt/freeware/bin/find ${mount}1/$host/$dbname -type f -mtime +$retnewday -exec /opt/freeware/bin/rm -vf {} \; >> $runlog
...
((timezoneoffset=`/opt/freeware/bin/date -u '+%Y%m%d%H%M%S'`-`/opt/freeware/bin/date '+%Y%m%d%H%M%S'`))
