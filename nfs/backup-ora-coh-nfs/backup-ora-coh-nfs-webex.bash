#!/bin/bash
#
# Name:         backup-ora-coh-nfs-webex.bash
#
...
function show_usage {
echo "usage: backup-ora-coh-nfs-webex.bash -r <Target connection> -c <Catalog connection> -h <host> -d <rac-node1-conn,rac-node2-conn,...> -o <Oracle_sid> -a <archive only> -i <incremental level> -y <Cohesity-cluster> -v <view> -m <mount-prefix> -n <number of mounts> -p <number of channels> -e <retention> -l <archive log keep days> -z <section size> -b <ORACLE_HOME> -w <yes/no>"
echo " "
...
if [[ -n $cohesityname ]]; then
   if [[ -z $view ]]; then
     echo "Cohesity view is not provided. exit" 
     exit 1
   fi
fi
