...
# 01/10/2021 Diana Yang   Change awk to /opt/freeware/bin/gawk, date to /opt/freeware/bin/date, change "find" to "/opt/freeware/bin/find"
#
#################################################################

function show_usage {
echo "usage: aix-duplicate-ora-coh-nfs.bash -r <Target connection> -e <Catalog connection> -b <Source host> -d <Source Oracle database> -a <Target host> -t <Target Oracle_DB_Name> -l <file contain duplicate settting> -m <mount-prefix> -n <number of mounts> -p <number of channels> -o <ORACLE_HOME> -c <source PDB> -f <yes/no> -w <yes/no>" 
echo " "
echo " Required Parameters"
echo " -r : Target connection (example: \"<dbuser>/<dbpass>@<target db connection>\")"
echo " -b : Source host - Oracle database host that the backup was run." 
echo " -d : Source Oracle_DB_Name (database backup was taken). If it is not RAC, it is the same as DB name. If it is RAC, it is the instance name like cohcdba2"
echo " -t : Target Oracle instance name (Oracle duplicate database)"
...
DATE_SUFFIX=`/opt/freeware/bin/date '+%Y%m%d%H%M%S'`
#echo $DATE_SUFFIX
...
if [[ ! -d "${mount}1/$shost/$sdbname/archivelog" ]]; then
   echo "Directory ${mount}1/$shost/$sdbname/archivelog does not exist, no backup files. Check the input"
   exit 1
fi
...
if [[ $shost = $thost ]]; then
   ...
   if [[ -z `grep -i db_file_name_convert $dbpfile` ]]; then
	 ...
	 if [[ -z ${ora_pfile} && -n `grep -i db_create_file_dest $dbpfile` ]]; then
	   if [[ -z `grep -i log_file_name_convert $dbpfile` ]]; then
	      echo "log_file_name_convert is not defined in init file $dbpfile"
              read -p "Continue may overwrite the target redo files. Do you want to continue? " answer1
	      if [[ $answer1 = [Nn]* ]]; then
	         exit 1
	      fi
	   fi
...
runoid=`ps -ef | grep pmon | /opt/freeware/bin/gawk 'NF>1{print $NF}' | grep -i $toraclesid | /opt/freeware/bin/gawk -F "_" '{print $3}'`
...
if [[ $preview = [Yy]* ]]; then
   ...
   if [ $? -eq 0 ]; then
      ...
      if [ $? -ne 0 ]; then
         echo "duplication failed, check rman log output $drmanlog"
         exit 1
	  fi
