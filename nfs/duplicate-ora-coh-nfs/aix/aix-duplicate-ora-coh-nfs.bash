#!/bin/bash
#
# Name:         aix-duplicate-ora-coh-nfs.bash
#
...
# 01/10/2021 Diana Yang   Change awk to /opt/freeware/bin/gawk, /bin/date to /opt/freeware/bin/date
# 04/15/2022 Diana Yang   Change the input plugin to be consistent with restore script. Major syntax changes 
# 04/25/2022 Diana Yang   Add noresume option 
#
#################################################################

function show_usage {
echo "usage: aix-duplicate-ora-coh-nfs.bash -r <Source ORACLE connection> -h <backup host> -c <Catalog ORACLE connection> -i <Target Oracle_DB_Name> -d <Source Oracle database> -b <file contain duplicate settting> -m <mount-prefix> -n <number of mounts> -t <point-in-time> -e <sequence> -p <number of channels> -o <ORACLE_HOME> -u <source PDB> -f <yes/no> -s <noresume> -w <yes/no>" 
echo " "
...
echo " -u : pluggable database (if this input is empty, it is CDB database restore"
...
while getopts ":r:c:h:i:d:t:e:b:m:n:p:o:u:f:s:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;; 
    c ) catalogc=$OPTARG;;
    h ) shost=$OPTARG;;
...
if [[ -n $itime ]]; then
   if [[ -n $ora_pfile ]];then
      itime1=`grep to_date $ora_pfile | grep -v "#" |  /opt/freeware/bin/gawk -F "'" '{print $2}'`
   ...
   if [[ -n $ora_pfile ]];then
      itime=`grep to_date $ora_pfile | grep -v "#" |  /opt/freeware/bin/gawk -F "'" '{print $2}'`
      echo "itime is $itime"

      if [[ -z $itime ]]; then
         ptime=`/opt/freeware/bin/date '+%Y%m%d%H%M%S'`
...
dirtimelist=`ls -d datafile* | /opt/freeware/bin/gawk -F "." '{print $2}'`
d=0
...
if [[ ! -z $ora_spfile ]]; then
  if test -f $ora_spfile; then
    ...
    if [[ $shost == $thost ]]; then
       if [[ $noresume = [Yy]* ]]; then
          echo " noresume;" >> $drmanfiled
       else
          echo ";" >> $drmanfiled
       fi
    else
       if [[ $noresume = [Yy]* ]]; then
          echo " noresume nofilenamecheck;" >> $drmanfiled
       else
  ...
  if [[ -z $pdbname ]]; then
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
  	   echo "duplicate target database to '$toraclesid' BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
	else
           echo "duplicate target database to '$toraclesid' BACKUP LOCATION '$backup_location';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate target database to '$toraclesid' BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
	else
     ...
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate target database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
	else 
           echo "duplicate target database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate target database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
	else
...
if [[ ! -z $ora_spfile ]]; then
  ...
  if [[ -z $pdbname ]]; then
     ...
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate database '$sdbname' to '$toraclesid' pluggable database '$spdbname' noresume;" >> $drmanfiled
	else 
           echo "duplicate database '$sdbname' to '$toraclesid' pluggable database '$spdbname';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate database '$sdbname' to '$toraclesid' pluggable database '$spdbname' noresume nofilenamecheck;" >> $drmanfiled
	else
...
echo "Database duplicate started at " `/opt/freeware/bin/date '+%Y%m%d%H%M%S'`
...
if [[ `grep -i error ${drmanlog}` ]]; then
   if [[ `grep -i "fs0 not allocated" $drmanlog` ]]; then
      echo "Database duplicate finished at " `/opt/freeware/bin/date '+%Y%m%d%H%M%S'`
      if [[ -z $targetc && -z $catalogc ]];then
      ...
      echo "Database duplicatep failed at " `/opt/freeware/bin/date '+%Y%m%d%H%M%S'`
      echo clean the softlink
     if [[ -n ${backup_location} ]]; then
         /bin/rm -r ${backup_location}/*
      fi
...
Please make sure the auxiliary database is shutdown and the files associated with database $toraclesid removed before the resun.
