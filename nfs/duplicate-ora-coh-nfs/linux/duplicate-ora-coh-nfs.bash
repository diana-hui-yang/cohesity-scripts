...
if [[ ${runoid} != ${toraclesid} ]]; then
   ...
   $sqllogin << EOF
   startup nomount pfile=${ORACLE_HOME}/dbs/init${toraclesid}.ora
EOF
else
   if [[ $force = [Yy]* ]]; then
  ...
  if [[ -z $pdbname ]]; then
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
  	   echo "duplicate target database to '$toraclesid' BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
           ...
           echo "dduplicate target database to '$toraclesid' BACKUP LOCATION '$backup_location';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
     ...
     if [[ $shost == $thost ]]; then
	   ...
	   echo "duplicate target database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
           ...
           echo "dduplicate target database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' nofilenamecheck;" >> $drmanfiled
	fi
     fi
  fi
fi  
