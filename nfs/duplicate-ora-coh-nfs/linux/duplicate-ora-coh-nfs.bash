...
if [[ -n $itime ]]; then
   if [[ -n $ora_pfile ]];then
      itime1=`grep to_date $ora_pfile | grep -v "#" |  awk -F "'" '{print $2}'`
...
if [[ ! -z $ora_spfile ]]; then
  if test -f $ora_spfile; then
    if [[ -z $pdbname ]]; then
       echo "duplicate database to '$toraclesid' BACKUP LOCATION '$backup_location'" >> $drmanfiled
    else
       echo "duplicate database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location'" >> $drmanfiled
    fi
    if [[ $shost == $thost ]]; then
       if [[ $noresume = [Yy]* ]]; then
  ...
  if [[ -z $pdbname ]]; then
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
  	   echo "duplicate database to '$toraclesid' BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
	else
           echo "duplicate database to '$toraclesid' BACKUP LOCATION '$backup_location';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate database to '$toraclesid' BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
	else
           echo "duplicate database to '$toraclesid' BACKUP LOCATION '$backup_location' nofilenamecheck;" >> $drmanfiled
	fi
     fi
  else
     if [[ $shost == $thost ]]; then
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' noresume;" >> $drmanfiled
	else 
           echo "duplicate database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location';" >> $drmanfiled
	fi
     else
        if [[ $noresume = [Yy]* ]]; then
	   echo "duplicate database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' noresume nofilenamecheck;" >> $drmanfiled
	else
           echo "duplicate database to '$toraclesid' pluggable database '$pdbname' BACKUP LOCATION '$backup_location' nofilenamecheck;" >> $drmanfiled
	fi
     fi
  fi
fi  
