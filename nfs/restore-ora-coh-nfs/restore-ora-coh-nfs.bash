...
function show_usage {
echo "usage: restore-ora-coh-nfs.bash -r <RMAN login> -h <backup host> -i <Oracle instance name> -d <Oracle_DB_Name> -c <file contain restore settting> -t <point-in-time> -l <yes/no> -m <mount-prefix> -n <number of mounts> -p <number of channels> -o <ORACLE_HOME> -f <yes/no> -g <yes/no> -w <yes/no>"
echo " "
...
if [[ ! -d "${mount}1/$bhost/$dbname/controlfile" ]]; then
   echo "Directory ${mount}1/$bhost/$dbname/controlfile does not exist, no backup files"
   exit 1
fi
...
#echo ${mount}1/$bhost/$dbname/archivelog
