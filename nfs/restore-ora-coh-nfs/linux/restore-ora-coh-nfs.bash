...
# find current server timezone difference from UTC time
  ((ttimezoneoffset=`/bin/date -u '+%Y%m%d%H%M%S'`-`/bin/date '+%Y%m%d%H%M%S'`))
...
if [[ -z $itime ]]; then
  ((ptime=`/bin/date '+%Y%m%d%H%M%S'`+$ttimezoneoffset-$stimezoneoffset))
...
while IFS= read -r line; do
   ...
   bitime=`echo $line | awk '{print $6 " " $7 " " $8}'`
   bfile=`echo $line | awk '{print $NF}'`
   ((btime=`/bin/date -d "$bitime" '+%Y%m%d%H%M%S'`+$ttimezoneoffset-$stimezoneoffset))
