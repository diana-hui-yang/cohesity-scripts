...
# 05/19/21 Diana Yang   All set newname during restore
...
echo $rmanlogin $oraclesid, $mount, $host, $num
...
if [[ ! -d "${mount}1/$host/$dbname/controlfile" ]]; then
   echo "Directory ${mount}1/$host/$dbname/controlfile does not exist, no backup files"
   exit 1
fi
...
#echo ${mount}1/$host/$dbname/archivelog
...
if [[ ! -d "${mount}1/$host/$dbname/controlfile" ]]; then
   echo "Directory ${mount}1/$host/$dbname/controlfile does not exist, no controlfiles backup"
   exit 1
fi
cd ${mount}1/$host/$dbname/controlfile
...
#     echo ptime $ptime
   if [[ $ptime -lt $btime ]]; then
     controlfile=${mount}1/$host/$dbname/controlfile/$bfile
	 oribtime=${btime::${#btime}-2}
     break
   else
     controlfile1=${mount}1/$host/$dbname/controlfile/$bfile
...
EOF
   ...
   echo "spfile restore started at " `/bin/gdate '+%Y%m%d%H%M%S'`
   if [[ -z $catalogc ]]; then
...
if [ $? -ne 0 ]; then
  echo "   "
  echo "restore controlfile $controlfile failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  ...
  echo "  "
  echo "restore controlfile finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
fi
...
echo "Database restore started at " `/bin/gdate '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
    echo "   "
    echo "Database restore validate failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
    ls -l ${ORACLE_HOME}/dbs/spfile*
	echo "The last 10 line of rman log output"
    echo " "
    ...
    echo "  "
    echo "Database restore validate finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
fi
...
EOF
   ...
   if [ $? -ne 0 ]; then
     echo "   "
     echo "Database restore failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
     ls -l ${ORACLE_HOME}/dbs/spfile*
     echo "The last 10 line of rman log output"
     echo " "
     echo "rmanlog file is $restore_rmanlog"
     ...
     echo "  "
     echo "Database restore finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
   fi
else
