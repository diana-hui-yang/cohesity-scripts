...
EOF
   ...
   if [[ -n $dbcopyfile ]]; then
      ...
      echo "The original Host is $orighost"
      echo "The original Host is $orighost" >> $runlog
  ...
  if [[ -n $orighost ]]; then
     if [[ -d "${mount}1/$orighost/$origdbname" ]]; then
        ...
        if [[ -d ${mount}1/$orighost/$origdbname/datafile ]]; then
           /opt/freeware/bin/find ${mount}1/$orighost/$origdbname/datafile/* -type f -mtime +1 | grep -v D-${dbname^^} >> ${origfilelist}
        fi
...
i=0
while [ $i -le 10 ]; do
   if [[ ! -d /$view/${backup_dir}/datafile.$DATE_SUFFIX ]]; then
      tgt_datafile_dir=/$view/${backup_dir}/datafile.$DATE_SUFFIX
      let i=10
   else
      echo "target clone directory $view/${backup_dir}/datafile.$DATE_SUFFIX was created by other methods"
      echo "target clone directory $view/${backup_dir}/datafile.$DATE_SUFFIX was created by other methods" >> $runlog
      DATE_SUFFIX=$[$DATE_SUFFIX+500]
      echo "A new target clone directory $view/${backup_dir}/datafile.$DATE_SUFFIX will be used"
      echo "A new target clone directory $view/${backup_dir}/datafile.$DATE_SUFFIX will be used" >> $runlog
   fi
   i=$[$i+1]
done
...
if [[ -z $apikey ]]; then 
   $pyname -v $cohesityname -u $cohesityuser -d $addomain -s $src_datafile_dir -t $tgt_datafile_dir
else
   if [[ $apikey = [Yy]* ]]; then
       $pyname -v $cohesityname -u $cohesityuser -i -s $src_datafile_dir -t $tgt_datafile_dir
   else
       $pyname -v $cohesityname -u $cohesityuser -d $addomain -s $src_datafile_dir -t $tgt_datafile_dir
   fi
fi
...
if [[ $archivelogonly = "yes" ]]; then
  ...
  if [[ $preview = "yes" || $preview = "Yes" || $preview = "YES" ]]; then
    ...
    if [[ -z $apikey ]]; then 
       ...
       if [[ $apikey = [Yy]* ]]; then
          echo $pyname -v $cohesityname -u $cohesityuser -i -s /$view/${backup_dir}/datafile -t /$view/${backup_dir}/datafile.$DATE_SUFFIX
       else
          echo $pyname -v $cohesityname -u $cohesityuser -d $addomain -s /$view/${backup_dir}/datafile -t /$view/${backup_dir}/datafile.$DATE_SUFFIX
       fi
    fi
