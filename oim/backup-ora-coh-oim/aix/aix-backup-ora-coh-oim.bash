...
# 03107/22 Diana Yang   change "find" to "/opt/freeware/bin/find"
#
#################################################################
...
#trim log directory
/opt/freeware/bin/find $DIR/log/$host -type f -mtime +7 -exec /bin/rm {} \;
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
 ...
 # let retnewday=$retday+1
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    ...
    /opt/freeware/bin/find ${mount}1/$host/$dbname/archivelog -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
    /opt/freeware/bin/find ${mount}1/$host/$dbname/controlfile -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
    /opt/freeware/bin/find ${mount}1/$host/$dbname/spfile -type f -mtime +$retnewday  -exec /bin/rm -v {} \; >> $runlog
    /opt/freeware/bin/find ${mount}1/$host/$dbname/* -type d -mtime +$retnewday -prune -exec rm -rv {} \; >> $runlog
 ...
 #   filenum=`wc -l ${filelist} | /opt/freeware/bin/gawk '{print $1}'`
    ...
    /opt/freeware/bin/find ${mount}1/$host/$dbname -depth -type d -empty -exec /bin/rmdir -v {} \; >> $runlog
  fi
