...
while getopts ":r:c:h:d:o:a:t:y:v:u:g:z:m:n:p:s:e:l:f:b:w:" opt; do
  case $opt in
    r ) targetc=$OPTARG;;
    c ) catalogc=$OPTARG;;
    h ) host=$OPTARG;;
...
DATE_SUFFIX=`/bin/gdate '+%Y%m%d%H%M%S'`
#echo $DATE_SUFFIX
...
echo \"Catalog the snapshot files started at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
export ORACLE_HOME=$oracle_home
export PATH=$PATH:$ORACLE_HOME/bin
export NLS_DATE_FORMAT='DD:MM:YYYY-HH24:MI:SS'
export ORACLE_SID=$yes_oracle_sid
...
Catalog the snapshot files finished at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
   echo \"$catalog_log does not exist. 
Catalog the snapshot files finished at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\` >> $runlog
   exit 1
fi

if [[ ! -z \`grep -i error" $catalog_log"\` ]]; then
  echo \"catalog the snapshot files failed at \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
  echo \"catalog the snapshot files failed at \" \`/bin/gdate '+%Y%m%d%H%M%S'\` >> $runlog
  exit 1
else
  echo \"Catalog the snapshot files finished at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
  echo \"Catalog the snapshot files finished at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\` >> $runlog
fi
" >> $catalog_bash
...
echo "Database backup started at " `/bin/gdate '+%Y%m%d%H%M%S'`
echo "Database backup started at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
...
if [[ -z $catalogc ]]; then
   ...
   @$rmanfiled1
EOF
   echo "Database Full or incremental backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
  echo "Database incremenal merge failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Database incremenal merge failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
  ...
  echo "Database incremenal merge finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Database incremenal merge finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog 
fi
...
echo "Archive logs backup started at " `/bin/gdate '+%Y%m%d%H%M%S'`
echo "Archive logs backup started at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
...
if [ $? -ne 0 ]; then
  echo "Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
  while IFS= read -r line
  do
    echo $line
  done < $rmanloga
  ...
  echo "Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog 
fi
}
...
echo "Archive logs backup started at " `/bin/gdate '+%Y%m%d%H%M%S'`
echo "Archive logs backup started at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runrlog
...
if [ $? -ne 0 ]; then
  echo "Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runrlog
  while IFS= read -r line
  do
    echo $line
  done < $rmanlogar
  ...
  echo "Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runrlog 
fi
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
 ...
 # let retnewday=$retday+1
  ...
  echo "Clean backup files older than $retnewday started at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Clean backup files older than $retnewday started at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
 ...
 #   filenum=`wc -l ${filelist} | gawk '{print $1}'`
  ...
  echo "Clean backup files older than $retnewday finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Clean backup files older than $retnewday finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
fi
...
echo "
clone directory /$view/${backup_dir}/datafile started at " `/bin/gdate '+%Y%m%d%H%M%S'`
echo "
clone directory /$view/${backup_dir}/datafile started at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
$pyname -v $cohesityname -u $cohesityuser -d $addomain -s /$view/${backup_dir}/datafile -t /$view/${backup_dir}/datafile.$DATE_SUFFIX

if [ $? -ne 0 ]; then
  echo "Cohesity snapshot backup files in ${mount}1/${backup_dir}/datafile failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Cohesity snapshot backup files in ${mount}1/${backup_dir}/datafile failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
  exit 1
else
  echo "Cohesity snapshot backup files in ${mount}1/${backup_dir}/datafile finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "Cohesity snapshot backup files in ${mount}1/${backup_dir}/datafile finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
fi
