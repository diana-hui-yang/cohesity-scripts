...
if [[ ! -z \`grep -i error" $catalog_log"\` ]]; then
  echo \"
catalog the snapshot files failed at \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
  echo \"
catalog the snapshot files failed at \" \`/bin/gdate '+%Y%m%d%H%M%S'\` >> $runlog
  exit 1
else
  echo \"
Catalog the snapshot files finished at  \" \`/bin/gdate '+%Y%m%d%H%M%S'\`
...
EOF
   echo "
Database Full or incremental backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
  echo "
Database incremenal merge failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Database incremenal merge failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
  ...
  echo "
Database incremenal merge finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Database incremenal merge finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog 
fi
...
if [ $? -ne 0 ]; then
  echo "
Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog
  while IFS= read -r line
  do
    echo $line
  done < $rmanloga
  ...
  echo "
Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runlog 
fi
}
...
Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Archive logs backup failed at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runrlog
  while IFS= read -r line
  do
    echo $line
  done < $rmanlogar
...
Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'`
  echo "
Archive logs backup finished at " `/bin/gdate '+%Y%m%d%H%M%S'` >> $runrlog 
fi
