...
echo " -g : Active Directory Domain of the Cohesity Oracle user. If the user ic created on Cohesity, use local as the input"
...
$pyname -s $cohesityname -u $cohesityuser -d $addomain -sp /$view/${backup_dir}/datafile -dp /$view/${backup_dir} -nd datafile.$DATE_SUFFIX
...
if [[ $archivelogonly = "yes" ]]; then
  ...
  if [[ $preview = "yes" || $preview = "Yes" || $preview = "YES" ]]; then
    ...
    echo $pyname -s $cohesityname -u $cohesityuser -d $addomain -sp /$view/${backup_dir}/datafile -dp /$view/${backup_dir} -nd datafile.$DATE_SUFFIX
    echo "---------------"
  else
    backup
    catalog_snapshot
