...
if [[ -z $catalogc ]]; then
   ...
   @$rmanfiled1
EOF
   echo "Database Full or incremental backup finished at " `/bin/date '+%Y%m%d%H%M%S'`
...
if [ $? -ne 0 ]; then
  echo "Database incremenal merge failed at " `/bin/date '+%Y%m%d%H%M%S'`
  echo "Database incremenal merge failed at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog
  ...
  echo "Database incremenal merge finished at " `/bin/date '+%Y%m%d%H%M%S'`
  echo "Database incremenal merge finished at " `/bin/date '+%Y%m%d%H%M%S'` >> $runlog 
fi
