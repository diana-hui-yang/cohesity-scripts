...
filelistdatafile=$DIR/log/$host/${dbname}.files.datafile.$DATE_SUFFIX
...
if ! [[ "$retday" =~ ^[0-9]+$ ]]; then
  ...
  if [[ -d "${mount}1/$host/$dbname" ]]; then
    find ${mount}1/$host/$dbname/datafile -type f -mtime +1 | grep -v D-${dbname^^} >> ${filelistdatafile}
    find ${mount}1/$host/$dbname -type f -mtime +$retnewday | grep -v D-${dbname^^} >> ${filelist}
